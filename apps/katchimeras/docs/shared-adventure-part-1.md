...
1. After the Garden’s first restoration, Mossprout notices a distant glint. Existing tutorial actions and saves remain valid.
2. After Steppling’s lesson, the Kingdom introduction names Heartwood and the signal. The player chooses what it promises: welcome, rest, or company.
...
`SHARED_ADVENTURE_ENABLED` currently follows `__DEV__`. Production remains disabled pending native acceptance. Content and art are bundled, so the first session does not depend on downloads. The post reuses the existing lantern asset.
...
The egg questions are unchanged. The post-hatch overlays now introduce the seed Mossprout kept, establish the player's preferred kind of support through a shared adventure scenario, notice an ambiguous glint after restoration, and end with Steppling's three-notch trail marker. The farewell recalls the saved support choice. Choice IDs, seed mapping, Bond awards, and tutorial checkpoints remain stable; previously saved transcript entries retain their original words.
