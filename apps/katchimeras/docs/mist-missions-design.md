...
Status: partly built (Sept 2026). Built: the opening plays on its own mission board (`features/onboarding/opening-mission-state.ts`, `use-opening-mission-board.ts`), and Steppling's misted clearing is cleared the same way (`features/onboarding/steppling-mission.ts`, `components/katchadeck/world/hatchable-mission-dock.tsx`): tapping the bubble at `gateway.pay` opens the shared purchase panel with the tile's price; confirming pays a mission ticket (`payHatchableMission`, the Glow flies into the tile) and only then records `glow.mission.paid`, which frames the tile with the opening's camera and docks a 5×4 board with three Socks, a sleeper and five veiled cells (see the revised section below); eight strikes fill the bar, the final item flies into the mist, and its impact records `glow.mission.cleared`, after which the reveal (free: the ticket was the price), the Egg and the hatch run unchanged. The shared dock is `MistMissionDock` in `kingdom-opening-merge-dock.tsx`; each mission keeps its own store with its own merge count. Not built: parcel rewards, a data-authored mission catalogue, missions for the six island campaigns.
