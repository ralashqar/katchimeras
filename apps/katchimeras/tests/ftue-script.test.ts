...
test('all opening answers are inclusive, stable, and produce the correct personal Seed', () => {
  ...
  assert.equal(mossproutFtueAction('companion.water_together', 'companion.choose_garden_return')?.nextStepId, 'companion.first_grow');
...
test('old Basket checkpoints migrate to the meditation bridge without locking Back', () => {
  ...
  assert.match(provider, /status === 'complete'[\s\S]*?await startGlowDiscovery[\s\S]*?await dismissFtueContentFlow[\s\S]*?await resumeActiveContentFlows/);
});
...
test('FTUE starts a relationship before the Garden, shows First Bloom, and continues through the resident lesson', () => {
  ...
  const transition = readFileSync(require.resolve('@incubator/presentation/screen-transition'), 'utf8');
