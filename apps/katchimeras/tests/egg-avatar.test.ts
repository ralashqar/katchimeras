...
import { existsSync, readFileSync } from './helpers/content-fs';
...
test('hero-resolution avatar art has a distinct high-resolution source', () => {
  ...
  assert.match(generatedAssets, /fullSource: require\('@incubator\/art-egg-avatars\/bases\/classic\.webp'\),/);
  assert.match(generatedAssets, /highSource: require\('@incubator\/art-egg-avatars\/bases\/high\/classic\.webp'\),/);
  assert.match(generatedAssets, /highSource: require\('@incubator\/art-egg-avatars\/faces\/high\/classic-smile\.webp'\),/);
