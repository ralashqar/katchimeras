...
test('every fifth order fills a crate: its bonus comes with that order, once', () => {
  ...
  for (let served = 0; served < SUPPLY_CRATE.every; served += 1) {
    ...
    world = reduceMergeWorld(world, { type: 'completeSupplyOrder', slot, index, timber: order.timber, glow: order.reward.coins, meals: order.meals, crate: SUPPLY_CRATE, now: 2_000 + served }).state;
    timber += order.timber;
    meals += order.meals;
  }
  ...
  assert.equal(world.materials?.meals, meals + SUPPLY_CRATE.meals, 'and its bonus Meals');
});
