...
test('a friend’s daily cards and photo feed are content, and the Kingdom begins a woken tile’s story from its bubble', () => {
  for (const definition of HATCHABLE_COMPANIONS) {
    ...
    if (definition.daily.photo) assert.ok(definition.daily.photo.category && definition.daily.photo.lines.thanks.length > 0 && definition.daily.photo.lines.noMatch && definition.daily.photo.camera.title, `${definition.companion}: the photo card has its words`);
  ...
  assert.match(card, /params: \{ companionActivityId: capture\.id, companionActivityFor: companion, companionReturnTo: returnTo \}/, 'the camera is told whose photo it is');
  const question = readFileSync('components/katchadeck/world/companion-daily-question.tsx', 'utf8');
  assert.match(question, /const prefix = `\$\{companion\}:poll:`;/, 'the day’s question is one of the friend’s scenario polls');
