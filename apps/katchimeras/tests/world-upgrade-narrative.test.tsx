...
test('required splash alternates portraits and cannot close before final saved Continue', async () => {
  ...
  const module = loadNativeModule('components/katchadeck/world/world-upgrade-narrative.tsx', {
    ...
    '@/features/onboarding/ftue-runtime': { useFtueRun: () => ({ status: ftueStatus }) },
  ...
  const speech = () => tree!.root.findAllByType(host('Pressable')).find((node) => node.props.accessibilityLabel === 'Continue dialogue')!;
  ...
  assert.equal(tree!.root.findAllByType(host('Button')).length, 0, 'intermediate Continue stays inside the current speech bubble');
  await act(async () => { speech().props.onPress(); speech().props.onPress(); });
  ...
  fail = true; await act(async () => speech().props.onPress());
  ...
  await act(async () => speech().props.onPress());
  ...
  ftueStatus = 'active';
  await act(async () => { tree = create(<Narrative {...props} required={false} />); });
  assert.equal(tree!.root.findAllByProps({ accessibilityLabel: 'Close story history' }).length, 0, 'FTUE history has neither X nor outside-dismiss target');
  await act(async () => tree!.root.findByType(host('Modal')).props.onRequestClose());
  assert.equal(closed, 2, 'FTUE blocks system Back even when surface navigation is unlocked');
  ftueStatus = 'complete';
  await act(async () => tree!.update(<Narrative {...props} required={false} />));
  assert.ok(tree!.root.findAllByProps({ accessibilityLabel: 'Close story history' }).length > 0, 'closing returns after FTUE');
  await act(async () => tree!.unmount());
  ...
  assert.equal(timers.size, 0, 'automatic dialogue timers are cancelled with the narrative');
});
