...
test('restoration waits for decoded target and finishes on the actual crossfade frame, even after a slow load', async () => {
  const clock = nativeMotionHarness();
  const module = loadNativeModule(require.resolve('@incubator/environments/hex-tile'), {}, {
...
for (const reducedMotion of [false, true]) {
  test(`Steppling Egg shares every tile reveal frame and stays asleep before feeding (reduced motion: ${reducedMotion})`, async () => {
    ...
    const Tile = loadNativeModule(require.resolve('@incubator/environments/hex-tile'), {}, globals, 'HavenUpgradeTileArt').HavenUpgradeTileArt as React.ComponentType<Record<string, unknown>>;
