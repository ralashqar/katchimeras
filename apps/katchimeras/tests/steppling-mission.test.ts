...
test('the Kingdom docks the mission under Steppling’s tile and clears the mist when its final item lands', () => {
  ...
  assert.match(screen, /const hatchableMist = useMistMission\(\{ active: stepplingMissionActive, mission: activeHatchable\.mission, owner: activeHatchable\.companion, tileNode: gatewayTileNode, boardMetrics: openingBoardMetrics, cameraSettled: ftueCameraSettled, glow: openingGlow, complete: completeActiveHatchable \}\);/, 'its own board and store, on the shared mission hook');
  assert.match(hook, /const store = useMissionBoard\(played\?\.storageKey \?\? NO_MISSION_STORAGE_KEY, active && played && owner \? played\.id : null, create, undefined, binding\);/, 'the store plays by the mission\u2019s mechanic');
  assert.match(hook, /finaleIdRef\.current = launchFinale\(from, definitionId, strike\);/);
  assert.match(hook, /const landed = finaleIdRef\.current != null && glow\.finaleLandedId === finaleIdRef\.current;/);
  assert.match(hook, /if \(!\(active && cleared && landed\)\) return;\s*const timer = setTimeout\(finish, WISP_FALL_MS\);/, 'the mist clears once the final item has struck the last wisp and it has fallen');
  ...
  assert.match(hook, /const cleared = Boolean\(played && mechanic && store\.mechanicState && mechanicComplete\(mechanic, played, store\.mechanicState\)\);[\s\S]*?if \(cleared\) finish\(\);/, 'a board saved with a full bar clears on resume');
  assert.match(screen, /<HatchableMissionDock mission=\{hatchableMist\.mission \?\? activeHatchable\.mission\}[\s\S]*?onFinale=\{hatchableMist\.onFinale\}/);
  ...
  assert.match(store, /type: result\.dreamEchoClearedId \? 'dream_echo_cleared' : 'merge_completed'/, 'a waking sends Glow like a merge');
  assert.match(dock, /const strike = result\?\.strike \?\? null;\s*if \(!result \|\| !strike\) return result;/, 'the store resolved the strike; the dock only flies it');
  ...
  assert.match(dock, /if \(strike\.finale\) \{[\s\S]*?setHiddenItemIds[\s\S]*?onFinale\?\.\(from, strike\.resultDefinitionId, strike\);/, 'the merge that fills the bar sends its item into the mist');
  assert.match(dock, /<MistMissionDock[\s\S]*?required=\{progress\.total\}/);
  assert.match(store, /const merged = command\.type === 'move' && result\.mergedCell != null;[\s\S]*?saveMission\(storageKey, activeRunId, result\.state, nextMerges, placedRef\.current, mechanicSaveState\(nextMechanic\)\);/, 'every merge is counted and saved with the board, with what its mechanic remembers');
