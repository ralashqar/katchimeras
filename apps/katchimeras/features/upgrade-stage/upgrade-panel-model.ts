...
export function lanternUpgradeModel(progress: LanternWorldProgress | undefined): UpgradePanelModel {
  ...
  return {
    ...
    benefits: next ? [
      { id: 'residents', label: 'Resident Wisps', from: entry.residents, to: next.residents },
      next.level === 2
        ? { id: 'bonus', label: 'Bonus pack', detail: `Every ${LANTERN_RECURRING_ORDERS} Garden orders` }
        : { id: 'rare', label: 'Bonus packs', detail: 'Rare or better guaranteed' },
    ] : [],
    requirements: next ? [
