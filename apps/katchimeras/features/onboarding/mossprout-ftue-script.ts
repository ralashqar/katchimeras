...
import type { FtueScriptDefinition, FtueStepDefinition } from './ftue-types';
...
import { sharedEggZoom, SHARED_EGG_CLOSE_ZOOM, SHARED_EGG_REST_ZOOM, SHARED_EGG_ENTRY_ZOOM, SHARED_RESIDENT_SCREEN_ANCHOR_Y } from '@/components/katchadeck/world/shared-resident-presentation';
...
export const MOSSPROUT_FTUE_SCRIPT: FtueScriptDefinition = {
  ...
  steps: [
    ...
    },
    {
      ...
      actions: [{ id: 'world.clear_mist', title: 'Drive them back', description: 'Merge plants that shoot. Bring down every wisp.', icon: 'leaf.fill', presentation: 'observed_game_action', handlerId: 'merge_item_created', nextStepId: 'world.mist_lift' }],
      ...
      // The Last Clearing's first battle (`last-clearing.ts`): won when every wisp is down.
      edges: [{ event: { type: 'battle_won', battleId: FIRST_BATTLE_ID }, commitActionId: 'world.clear_mist', nextStepId: 'world.mist_lift' }],
    ...
    },
    {
      ...
      // The Mist retreats (beat 7): Mossprout's lines under the veil lift.
      guide: { eyebrow: '', title: MIST_RETREAT_LINES[0]!, body: MIST_RETREAT_LINES[1]! },
      // Committed by the Kingdom when the veil crossblend completes.
      actions: [{ id: 'world.mist_lifted', title: 'Continue', description: 'The Mist pulls back from the clearing.', icon: 'sparkles', presentation: 'acknowledgement', handlerId: 'acknowledgement', nextStepId: HEART_TREE_STEP_ID }],
      // Mossprout's ordinary interaction framing (the one a tap on him uses), not the old Egg close-up.
      camera: { kind: 'focus_target', target: { kind: 'haven_resident', characterId: 'mossprout' }, zoom: MOSSPROUT_WORLD_EGG_REST_ZOOM, anchorY: SHARED_RESIDENT_SCREEN_ANCHOR_Y, durationMs: 1400 },
    },
    {
      // The Heart Tree (beat 8): the camera pushes in on it, Mossprout asks, and the first light wakes it.
      id: HEART_TREE_STEP_ID, surface: 'haven', navigation: { lock: true, resume: { kind: 'haven' } },
      guide: { eyebrow: '', title: HEART_TREE_TITLE, body: HEART_TREE_LINES[0]! },
      actions: [{ id: HEART_TREE_ACTION_ID, title: HEART_TREE_RESTORE, description: 'Spend the first light to wake the Heart Tree.', icon: 'sparkles', presentation: 'cta_action', handlerId: 'acknowledgement', nextStepId: SANCTUARY_STEP_ID }],
      camera: { kind: 'focus_target', target: { kind: 'haven_garden_tile', characterId: 'mossprout' }, zoom: 1.28, anchorY: 0.5, durationMs: 1600 },
    },
    {
      // The Sanctuary (beat 9): the title card.
      id: SANCTUARY_STEP_ID, surface: 'haven', navigation: { lock: true, resume: { kind: 'haven' } },
      guide: { eyebrow: '', title: SANCTUARY_TITLE, body: SANCTUARY_LINE },
      actions: [{ id: SANCTUARY_ACTION_ID, title: 'Continue', description: 'The first Sanctuary in a very long time.', icon: 'sparkles', presentation: 'acknowledgement', handlerId: 'acknowledgement', nextStepId: FRONTIER_STEP_ID }],
      camera: { kind: 'focus_target', target: { kind: 'haven_garden_tile', characterId: 'mossprout' }, zoom: 1.28, anchorY: 0.5, durationMs: 900 },
    },
    {
      // The frontier (beat 10): a slow pull-out until the lit clearing is a speck in the Mist and the Hollow Tree shows.
      id: FRONTIER_STEP_ID, surface: 'haven', navigation: { lock: true, resume: { kind: 'haven' } },
      guide: { eyebrow: '', title: FRONTIER_TITLE, body: FRONTIER_LINES[0]! },
      actions: [{ id: FRONTIER_ACTION_ID, title: 'Continue', description: 'Nearly everything is Mist.', icon: 'sparkles', presentation: 'acknowledgement', handlerId: 'acknowledgement', nextStepId: LOST_TRACKS_STEP_ID }],
      camera: { kind: 'fit_targets', targets: [{ kind: 'haven_world' }], durationMs: 3_600 },
    },
    {
    ...
    ...LOST_TRAIL_STONE_STEP_IDS.map((stepId, index): FtueStepDefinition => ({
      ...
      camera: { kind: 'focus_target', target: { kind: 'haven_structure', structureId: LOST_TRAIL_TILE_ID }, zoom: OPENING_CAMERA_ZOOM, anchorY: OPENING_CAMERA_ANCHOR_Y, durationMs: 900 },
    })),
    {
    ...
    {
      // Steppling home: his tile in the Sanctuary opens with him on it, the reveal, and his first words.
      id: STEPPLING_MEETS_STEP_ID, surface: 'haven', navigation: { lock: true, resume: { kind: 'haven' } },
      guide: { eyebrow: '', title: STEPPLING_MEETS_TITLE, body: STEPPLING_MEETS_LINES[0]!.text },
      actions: [{ id: STEPPLING_MEETS_ACTION_ID, title: 'Welcome, Steppling', description: 'Steppling is home.', icon: 'sparkles', presentation: 'acknowledgement', handlerId: 'acknowledgement', nextStepId: STEPPLING_JOINED_STEP_ID }],
      camera: { kind: 'focus_target', target: { kind: 'haven_structure', structureId: 'steppling-home' }, zoom: 1.35, anchorY: 0.42, durationMs: 1_800 },
    },
    {
      // STEPPLING HAS JOINED YOUR SANCTUARY.
      id: STEPPLING_JOINED_STEP_ID, surface: 'haven', navigation: { lock: true, resume: { kind: 'haven' } },
      guide: { eyebrow: '', title: STEPPLING_JOINED_TITLE, body: '' },
      actions: [{ id: STEPPLING_JOINED_ACTION_ID, title: 'Continue', description: 'A second hero.', icon: 'sparkles', presentation: 'acknowledgement', handlerId: 'acknowledgement', nextStepId: HOME_STEP_ID }],
      camera: { kind: 'focus_target', target: { kind: 'haven_structure', structureId: 'steppling-home' }, zoom: 1.35, anchorY: 0.42, durationMs: 600 },
    },
    {
      // Home (beat 15): the Sanctuary, two now. The first session ends here.
      id: HOME_STEP_ID, surface: 'haven', navigation: { lock: true, resume: { kind: 'haven' } },
      guide: { eyebrow: '', title: HOME_TITLE, body: HOME_LINES[0]! },
      actions: [{ id: HOME_ACTION_ID, title: 'Continue', description: 'The first session is over.', icon: 'sparkles', presentation: 'acknowledgement', handlerId: 'acknowledgement', nextStepId: 'complete' }],
      camera: { kind: 'focus_target', target: { kind: 'haven_garden_tile', characterId: 'mossprout' }, zoom: 0.95, anchorY: 0.46, durationMs: 1_800 },
    },
    {
