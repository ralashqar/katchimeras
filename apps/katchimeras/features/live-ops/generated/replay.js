...
var HATCHABLE_COMPANIONS_BUNDLED = [STEPPLING_HATCHABLE, BARISTABBIT_HATCHABLE];
...
function validateUpgradeEffect(payload) {
  const targetError = validateStoryTarget(payload.target);
  if (targetError) return targetError;
  const target = payload.target;
  if (target.kind === "haven_structure" && (typeof target.structureId !== "string" || !sharedWorldPurchase(target.structureId) || payload.toLevel !== 1 || !["normal", "free"].includes(payload.economy?.mode ?? ""))) return "Unknown shared-world purchase";
...
function validateStoryNodeCapability(node) {
...
var { validateContentFlowDefinition, defineContentFlow } = createContentFlowCompiler({
  validateStoryNodeCapability,
...
function createHatchableDiscoveryFlow(definition) {
...
function createHatchableDayOneFlow(definition) {
...
function createHatchableGardenLessonFlow(definition) {
