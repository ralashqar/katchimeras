...
export const rulesetId = "merge-v1-22d414730357ed0cb8d4edbd9bf05332e713081d9d2de8d59f717758f002e6b0";
...
function parseReplayBatch(value) {
  ...
  exact(value, ["rulesetId", "epoch", "deviceId", "fromSequence", "actions"]);
...
function replayVerifiedBatch(checkpoint, input, serverNow) {
  ...
  for (const [index, action] of batch.actions.entries()) {
    ...
    const command = action.type === "tapGenerator" ? { ...action, now: serverNow, seed: `${checkpoint.seed}:${sequence}` } : { ...action, now: serverNow };
  ...
  return async (request) => {
    ...
    try {
      if (body.operation === "begin") {
        ...
        const data = await call("begin_verified_merge_v1", {
          device_id: body.deviceId,
          ruleset_id: deps.rulesetId,
          initial_state: initialVerifiedWorld(now),
      ...
      if (loaded.duplicate === true) return json(loaded);
      const checkpoint = loaded.checkpoint;
      if (checkpoint.rulesetId !== deps.rulesetId) return json({ ok: false, reason: "ruleset_unavailable" }, 409);
      let replayed;
      try {
        replayed = replayVerifiedBatch(checkpoint, batch, loaded.serverNow);
      } catch {
        return json({ ok: false, reason: "replay_rejected" }, 409);
      }
      const committed = await call("commit_verified_merge_v1", { batch, next_state: replayed.state, verified_events: replayed.events });
      return json(committed, committed.ok ? 200 : 409);
    } catch {
