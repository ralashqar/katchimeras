...
export const rulesetId = "merge-v1-2cb07fa3ee72a2b67de9d620f5657ed7ead102882b18e5e2ce661a7efe92bd50";
...
var FEASTLE_HATCHABLE = {
  ...
  "tile": {
    ...
    "name": "A warm table",
    "revealPreset": "mist-clear",
    "alphaBoundsKey": "floating_neighborhood_v2_feastle_haven_stage_4_hex_tile.webp",
    "markerLines": {
...
function mergeCommandEvents(before, command, result2, contentRevision) {
  ...
  const base = { version: 1, source: "merge-world", sourceRevision: result2.state.revision, occurredAt: command.now, contentRevision, quantity: 1 };
  ...
  if (command.type === "move" && result2.mergedCell != null) {
    const item = result2.state.board[result2.mergedCell]?.occupant;
    if (item?.kind === "item") events.push({ ...base, id: `${id}:merge`, kind: "merge", context: { itemId: item.definitionId, itemTier: Number(item.definitionId.split(":").at(-1)) || void 0, regionId: "mossprout-grove" } });
  }
  if (command.type === "serveOrder" && result2.servedOrderId) {
    ...
    events.push({ ...base, id: `${id}:order:${result2.servedOrderId}`, kind: "order_completed", context: { targetId: result2.servedOrderId, companionId: order8?.characterId, regionId: "mossprout-grove" } });
  }
  return events;
}
