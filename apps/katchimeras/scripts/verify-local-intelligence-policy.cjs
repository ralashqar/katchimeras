...
function transpile(relativePath, outputName) {
  const source = readVerificationSource(contentPath(root, relativePath), 'utf8');
...
(async () => {
  ...
  check('missing Foundation result preserves the note for manual review', local.intelligenceProvider === 'deterministic' && local.journalRoutes.length === 0);
  ...
  check('text opt-in cannot replace Foundation journal classification', remote.intelligenceProvider === 'deterministic' && remote.journalRoutes.length === 0 && calls.calls === 0);
  const remotePlaces = await places.resolvePlaceSeedsForDay({ locations: [], moments: [] }, [], { allowRemote: true });
  check('explicit place action still permits unrelated place enrichment', remotePlaces[0] === 'coffee_shop' && calls.calls === 1);
  const isolated = await note.interpretNote({ text: 'Foundation test' }, { allowRemote: true });
  check('Foundation-only note routing has no development bypass', isolated.journalRoutes.length === 0 && calls.calls === 1);
