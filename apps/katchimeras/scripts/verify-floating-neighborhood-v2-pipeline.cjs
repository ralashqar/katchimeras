...
const { existsSync, readFileSync: nativeReadFileSync, readdirSync } = require('node:fs');
...
const briefTemplate = JSON.parse(readFileSync(contentPath(root, 'design/floating-neighborhood-v2/new-environment-brief.json'), 'utf8'));
if (!briefTemplate.floor) {
  throw new Error('Floating-v2 brief template must require a theme-specific floor.');
}
const briefsDirectory = contentPath(root, 'design/floating-neighborhood-v2/briefs');
...
const generator = readFileSync(contentPath(root, 'scripts/generate-katchimera-hex-tile.py'), 'utf8');
...
const promotion = readFileSync(contentPath(root, 'scripts/promote-floating-neighborhood-v2-tile.py'), 'utf8');
