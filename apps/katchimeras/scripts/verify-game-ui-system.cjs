...
let failures = 0;
const read = (relative) => readVerificationSource(contentPath(root, relative), 'utf8');
...
function sourceFiles(relativeDirectory) {
  const directory = contentPath(root, relativeDirectory);
...
required.forEach((file) => check(`${file} exists`, fs.existsSync(contentPath(root, file))));
