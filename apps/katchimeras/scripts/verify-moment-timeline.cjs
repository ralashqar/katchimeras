...
function transpile(relativePath, outName) {
  const source = readVerificationSource(contentPath(projectRoot, relativePath), 'utf8');
...
const todayCategoriesSource = readVerificationSource(contentPath(projectRoot, 'utils/today-categories.ts'), 'utf8');
const journalSource = readVerificationSource(contentPath(projectRoot, 'components/katchadeck/home/day-journal-sections.tsx'), 'utf8');
const sanctuarySource = readVerificationSource(contentPath(projectRoot, 'components/katchadeck/world/sanctuary-sheet.tsx'), 'utf8');
const cardViewerSource = readVerificationSource(contentPath(projectRoot, 'components/katchadeck/cards/daily-card-viewer.tsx'), 'utf8');
