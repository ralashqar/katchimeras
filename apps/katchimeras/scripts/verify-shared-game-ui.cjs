...
const { existsSync, readFileSync: nativeReadFileSync } = require('node:fs');
...
for (const file of required) {
  try { readFileSync(contentPath(root, file)); } catch { failures.push(`Missing shared UI contract: ${file}`); }
}
for (const file of migratedSheets) {
  const source = readFileSync(contentPath(root, file), 'utf8');
...
if (existsSync(contentPath(root, 'components/katchadeck/ui/meadow-sheet.tsx'))) failures.push('Retired MeadowSheet compatibility wrapper was reintroduced');

const controller = readFileSync(contentPath(root, 'features/today/use-today-sheet-controller.ts'), 'utf8');
if (/useState\s*\(false\)/.test(controller)) failures.push('Today sheet controller reintroduced independent popup booleans');

const companionSheet = readFileSync(contentPath(root, 'components/katchadeck/world/companion-interaction-sheet.tsx'), 'utf8');
...
const companionPrimitives = readFileSync(contentPath(root, 'components/katchadeck/world/companion-interaction-primitives.tsx'), 'utf8');
...
const questHost = readFileSync(contentPath(root, 'components/katchadeck/world/quests/quest-experience-host.tsx'), 'utf8');
...
const moments = readFileSync(contentPath(root, 'components/katchadeck/world/sanctuary-sheet.tsx'), 'utf8');
...
const button = readFileSync(contentPath(root, 'components/katchadeck/ui/katcha-button.tsx'), 'utf8');
