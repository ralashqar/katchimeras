...
for (const relative of required) {
  if (!existsSync(contentPath(root, relative))) {
...
for (const character of managedCharacters) {
  const manifest = JSON.parse(
    readFileSync(contentPath(root, `design/floating-neighborhood-v2/haven/${character}/progression.json`), 'utf8'),
  );
...
const generator = readFileSync(contentPath(root, 'scripts/generate-haven-progression.py'), 'utf8');
...
const preparer = readFileSync(contentPath(root, 'scripts/prepare-haven-progression.py'), 'utf8');
for (const contract of [
  'matted.png',
  'normalize_matte',
...
const promotion = readFileSync(contentPath(root, 'scripts/promote-haven-progression.py'), 'utf8');
for (const contract of [
  '--candidate-dir',
  '--prepared-dir',
...
const worldVisuals = readFileSync(contentPath(root, 'utils/world-visuals.ts'), 'utf8');
for (const character of managedCharacters) {
  for (let stage = 0; stage < 5; stage += 1) {
    ...
    for (const suffix of ['.webp', '_512.webp', '_256.webp']) {
      if (!existsSync(contentPath(root, 'assets/images/katchimeras/world/hex', `${assetKey}${suffix}`))) {
    ...
    for (const suffix of ['source.png', 'alpha.png']) {
      ...
      if (!existsSync(contentPath(root, 'design/floating-neighborhood-v2', filename))) {
