...
function transpile(source, name) {
  const output = ts.transpileModule(readVerificationSource(contentPath(root, source), 'utf8'), {
...
(async () => {
  ...
  const foundationSource = readVerificationSource(contentPath(root, 'utils/foundation-scene.ts'), 'utf8');
  check(
  ...
  const evidenceSource = readVerificationSource(contentPath(root, 'utils/photo-journal-evidence.ts'), 'utf8');
  check(
  ...
  const devTraceSource = readVerificationSource(contentPath(root, 'utils/dev-photo-analysis.ts'), 'utf8');
  const intelligenceLabSource = readVerificationSource(contentPath(root, 'app/intelligence-lab.tsx'), 'utf8');
  check(
  ...
  const essenceReviewSource = readVerificationSource(contentPath(root, 'components/katchadeck/capture/essence-review.tsx'), 'utf8');
