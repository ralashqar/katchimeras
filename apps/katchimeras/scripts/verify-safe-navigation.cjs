...
const source = readVerificationSource(contentPath(root, 'utils/safe-navigation.ts'), 'utf8');
...
collect(contentPath(root, 'app'));
const unsafe = routeFiles.filter((file) => /router\.back\(\)|navigation\.goBack\(\)/.test(readVerificationSource(file, 'utf8')));
