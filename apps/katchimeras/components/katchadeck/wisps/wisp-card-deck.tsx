...
export function WispPackReveal({ pack, onFocus, onDone, pending, error, actionLabel, previewStartedAt }: {
  ...
  return <DiscoveryRewardSequence key={pack.id} backdrop={false}
    renderHero={size => <WispCollectionCard wispId={selected.wispId} width={size / 1.5} cardNumber={ids.indexOf(selected.wispId) + 1} />}
    ...
    <View style={styles.heading}><Text style={styles.eyebrow}>{album.name.toUpperCase()}</Text><Text accessibilityRole="header" style={styles.title}>{cards.length === 1 ? 'Your first Wisp card' : 'Your Wisp cards'}</Text><Text style={styles.summary}>{newCount} new {newCount === 1 ? 'discovery' : 'discoveries'}{echoes ? ` · +${echoes} Echoes` : ''}</Text></View>
    <CollectibleCardDeck cards={cards} selectedId={selected.id} onSelect={(_, index) => onFocus(index)} cardRatio={2 / 3} maxCardHeight={Math.min(430, Math.max(230, height * 0.5))} navigationLabel="Wisp pack"
      renderCard={(card, _, size) => <WispCollectionCard wispId={card.wispId} width={size.width} cardNumber={ids.indexOf(card.wispId) + 1} />}
