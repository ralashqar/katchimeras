...
export const REWARD_TOKEN_PAYOUT_DURATION_MS = REWARD_TOKEN_RISE_MS
...
export function RewardTokenFlight({
  ...
  }, [arrive, count, flightProgress, hoverPhase, index, reduceMotion, riseProgress]);
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    const baseX = flight === 0
      ? stagedX
      : inverse * inverse * burstX + 2 * inverse * flight * controlX + flight * flight * to.x;
    const baseY = flight === 0
      ? stagedY
      : inverse * inverse * burstY + 2 * inverse * flight * controlY + flight * flight * to.y;
...
const styles = StyleSheet.create({
  token: { alignItems: 'center', justifyContent: 'center', left: 0, position: 'absolute', top: 0 },
});
