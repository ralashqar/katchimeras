...
import { useHatchableDayOne } from '@/features/companion/use-steppling-day-one';
...
export function KatchimeraCompanionRouteScreen({ creatureId, source, ftueRouteOrigin = false, ftueConversationDefinitionId, journeyReturnConversationDefinitionId, hostedNarrativeRequired = false, residentStoryResumeRequested = false, renderRegularStage = false, reuseUnderlyingStage = false, hostedInHaven = false, onHostedClose, onHostedFtueComplete, onHostedInitialConversationComplete, onHostedOpenMerge, onVisibleCreatureRewardPulse }: {
  ...
  // A hatchable friend's page: their day one and their garden lesson, from their definition.
  ...
  }, [creatureId, hatchable, onHostedOpenMerge, router, stepplingDayOne.error, stepplingDayOne.gardenHandoffPending, stepplingDayOne.ready, surfaceActive, transitionTo]);
  ...
  const continueToMist = useCallback(async () => {
    ...
    try {
      ...
      setMistHandoffError(true);
    } finally { ftueHandoffRef.current = false; }
  }, [hostedInHaven, onHostedClose, router, transitionTo]);
  const pendingMistExit = useFtueMistHandoff({ run: ftueRun,
    active: surfaceActive && familyId === 'mossprout', handoffActive: mistHandoffActive, onHandoff: continueToMist });
  useEffect(() => {
    if (!surfaceActive) { setMistHandoffActive(false); setMistHandoffError(false); }
  }, [surfaceActive]);
  const completeFtueProfileStep = useCallback((nickname?: string) => {
    const run = loadFtueRun();
    if (run?.status !== 'active') return;
    if (run.stepId === 'companion.first_rest') {
      return completeFtueConversation();
    }
    if (run.stepId === 'companion.resident_match_result') {
  ...
  const acknowledgeFtueBond = useCallback(async () => {
    const run = loadFtueRun();
    if (run?.status === 'active' && run.stepId === 'companion.notice_bond_spotlight') {
      await advanceFtueActionDurably({ expectedStepId: 'companion.notice_bond_spotlight',
        actionId: 'companion.acknowledge_notice_bond', evidenceRef: 'mossprout-first-notice-bond-spotlight' });
      return;
    }
  ...
  if (hatchable && (!stepplingDayOne.ready || stepplingDayOne.error)) {
  ...
  if (hatchable && stepplingLesson.active && HATCHABLE_LESSON_FINALE_NODE_IDS.includes(stepplingLesson.run?.nodeId ?? '')) {
    return <StepplingGardenFinale definition={hatchable} hosted={hostedInHaven} summary={stepplingLesson.run?.nodeId === 'summary'} />;
  }
  return (
    ...
    <KingdomCompanionScreen
      ...
      onInitialConversationComplete={hostedInitialConversationId ? async (session) => {
        if (hatchable && hostedInitialConversationId === stepplingDayOne.definitionId) await completeStepplingNarrative();
      ...
      ftueProfileStep={ftueRun?.status === 'active' && ftueRun.stepId === 'companion.intro_action'
        ...
        : ftueRun?.status === 'active' && ftueRun.stepId === 'companion.bond_intro'
          ...
          : ftueRun?.status === 'active' && ftueRun.stepId === 'companion.garden_intro'
            ? 'garden_intro'
            : ftueRun?.status === 'active' && ftueRun.stepId === 'companion.notice_bond_spotlight'
              ? 'notice_bond'
            : ftueRun?.status === 'active' && isMossproutFirstGrowStep(ftueRun.stepId)
              ? 'first_grow'
            ...
            : ftueRun?.status === 'active' && ftueRun.stepId === 'companion.resident_match_result'
              ? 'resident_result'
            : null}
      ftueBondSpotlightActive={ftueRun?.status === 'active' && (ftueRun.stepId === 'companion.bond_spotlight' || ftueRun.stepId === 'companion.notice_bond_spotlight')}
      ...
      onCloseCompanion={() => {
        ...
        if (run?.status === 'active' && run.stepId === 'companion.meditating') {
          return completeFtueProfileStep();
        }
