...
type Props = {
  ...
  /** The hatchable companion's tile the gateway camera, the docked mission and the discovered Egg belong to. */
  gatewayTileId?: string;
...
const KINGDOM_DREAM_MIST_LOCK_SOURCE = require('@incubator/art-world/hex/kingdom_dream_mist_lock_v1_512.webp');
...
const WORLD_FTUE_CRACK_ONE = require('@incubator/art-egg-avatars/effects/crack-1.png');
const WORLD_FTUE_CRACK_TWO = require('@incubator/art-egg-avatars/effects/crack-2.png');
const WORLD_FTUE_SOFT_RING = require('@incubator/art-characters/soft-ring.png');
const WORLD_FTUE_SOFT_GLOW = require('@incubator/art-characters/soft-glow.png');
...
export const KingdomHexCanvas = memo(function KingdomHexCanvas({
  ...
  gatewayTileId = 'steppling-home',
  homeVeil = 'none',
  homeSolo = false,
  revealWorldWithHome = false,
  ...
  // A hatchable companion's tile clearing from the Mist: the Egg is revealed with it.
  ...
  const committedScene = useMemo(
    () => focusedMossproutWorld
      ? buildMossproutHexNeighborhoodScene(companionSlots, mossproutNatureIslandLevels ?? {
          ...
          'wildgrowth-grove': 0,
        }, mossproutGarden, mossproutNatureIslandReveals, { homeVeiled: homeVeil === 'veiled' || homeVeil === 'lifting', homeSolo, revealWorldWithHome })
      : buildKingdomHexScene(companionSlots, hexTileSelection.value, identity, verticalAlignmentSelection.value),
    [companionSlots, focusedMossproutWorld, hexTileSelection, homeSolo, revealWorldWithHome, homeVeil, identity, mossproutGarden, mossproutNatureIslandLevels, mossproutNatureIslandReveals, verticalAlignmentSelection]
  );
  ...
  const arrivingFriendTile = useMemo(() => {
    if (!upgradePresentation?.noEgg || !revealingHatchableTileId) return null;
    if (upgradePhase !== 'reveal' && upgradePhase !== 'react' && upgradePhase !== 'complete') return null;
    return committedScene.tiles.find((tile) => tile.kind === 'companion' && tile.companion?.kind === 'owned' && tile.companion.familyId === upgradePresentation.characterId) ?? null;
  }, [committedScene.tiles, revealingHatchableTileId, upgradePhase, upgradePresentation?.characterId, upgradePresentation?.noEgg]);
  const upgradeScene = useMemo(() => arrivingFriendTile && !upgradeFromScene.tiles.some((tile) => tile.id === arrivingFriendTile.id)
    ? { ...upgradeFromScene, tiles: [...upgradeFromScene.tiles, arrivingFriendTile] }
    : upgradeFromScene, [arrivingFriendTile, upgradeFromScene]);
  ...
  }, [gardenFocusFrame, gatewayTileId, initialTutorialCameraScale, scene.tileArtLayers, scene.tiles, tutorialCamera, viewport.height, worldSubjectPresentation?.growthProgress]);
  ...
  const camera = useKingdomHexCamera({
    ...
    magneticFocus: presentation?.focusMode === 'magnetic'
      ...
      : undefined,
    minimumScale: focusedMossproutWorld ? cameraMinimumScale ?? 0.28 : undefined,
  ...
  useEffect(() => {
    ...
    const layer = scene.tileArtLayers.find((candidate) => candidate.id === `structure:${gatewayTileId}`);
    if (!layer) return;
  ...
  useEffect(() => {
    ...
    if (target.kind === 'haven_heartwood_pair' && gardenFrame && sceneHomeTile) {
      const homeFrame = scene.tileArtLayers.find((layer) => layer.id === sceneHomeTile.id)?.frame;
      if (!homeFrame) return;
      appliedTutorialCameraRef.current = applicationKey;
      focusTutorialResident(
        (gardenFrame.left + gardenFrame.width / 2 + homeFrame.left + homeFrame.width / 2) / 2,
        (gardenFrame.top + gardenFrame.height / 2 + homeFrame.top + homeFrame.height / 2) / 2,
        { anchorY: tutorialCamera.anchorY, durationMs, zoom: tutorialCamera.zoom, unbounded: true },
      );
      return;
    }
  ...
  const interactionResidentProjection = useMemo(() => {
    ...
    // retires, even though it does not open a regular resident interaction.
    const tile = scene.tiles.find((candidate) => (
      candidate.kind === 'companion'
      && candidate.companion?.kind === 'owned'
      && (interactionResidentId
        ? candidate.companion.creature.creatureId === interactionResidentId
        : worldSubjectPresentation?.companionVisible && candidate.companion.familyId === 'mossprout')
    ));
  ...
  }, [artLayerById, creatureWorldSize, interactionNatureIslandId, interactionResidentId, scene.tiles, worldSubjectPresentation?.companionVisible]);

  // Residents already on the map when it loaded are simply there; one who joins later (a friend come home) arrives.
  const mountedResidentIdsRef = useRef<Set<string> | null>(null);
  if (mountedResidentIdsRef.current === null) mountedResidentIdsRef.current = new Set(scene.tiles.map((tile) => tile.id));
  ...
  return (
    <View collapsable={false} ref={rootRef} style={styles.root} onLayout={onLayout}>
                ...
                <Fragment key={`tile-stack-${layer.id}`}>
                  ...
                  {transitionLayers ? (
                    <HavenUpgradeTileArt
                      ...
                      sharedRevealProgress={transitionLayers.toLayer.id === `structure:${gatewayTileId}` ? stepplingRevealProgress
      ...
      {focusedMossproutWorld && (revealingStepplingEgg || (((mossproutGarden?.hatchableTiles?.[gatewayTileId] ?? mossproutGarden?.gateway) === 'egg' || discoveredEggInteraction) && !upgradePresentation && !storySceneGuard)) ? (() => {
        ...
        const layer = revealingStepplingEgg ? upgradeLayers?.toLayer
          : scene.tileArtLayers.find((candidate) => candidate.id === `structure:${gatewayTileId}`);
        ...
        return <RevealedCompanionEgg idleDiscovery={!discoveredEggInteraction} fullSize eggSkinId="moss" presentation={discoveredEggPresentation} targetRef={discoveredEggTargetRef}
          revealProgress={revealingStepplingEgg || settlingUpgrade?.layers.toLayer.id === `structure:${gatewayTileId}` ? stepplingRevealProgress : undefined}
...
type ResidentProps = {
  /** Joined the map after it loaded (a friend come home): arrives with a drop and a bounce. */
  entrance?: boolean;
  animated?: boolean;
...
const ResidentCreature = memo(function ResidentCreature({
  ...
  // `entrance` is fixed at mount; the arrival runs once.
...
const LostSilhouette = memo(function LostSilhouette({ frame, source }: { frame: { left: number; top: number; width: number; height: number }; source: ImageSourcePropType }) {
  ...
  useEffect(() => {
    ...
    presence.value = withRepeat(withSequence(
      withTiming(0.62, { duration: 1_900, easing: Easing.inOut(Easing.sin) }),
      withTiming(0.28, { duration: 2_300, easing: Easing.inOut(Easing.sin) }),
    ), -1, false);
