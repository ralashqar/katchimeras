...
type HatchableMissionDockProps = {
  ...
  landings?: GlowLandingSource & { launchVolley?: OpeningGlowStore['launchVolley']; launchBolts?: OpeningGlowStore['launchBolts'] };
 ...
 */
type BoltBox = { left: number; top: number; width: number; height: number };
type MistBolt = { id: number; from: BoltBox; to: BoltBox; delay: number; onImpact: () => void };
const MIST_BOLT_MS = 520;
const MIST_BOLT_REACH = 0.25;
const MIST_BOLT_LEAD_MS = 90;
const MIST_BOLT_STAGGER_MS = 110;
const MIST_BOLT_SEGMENTS = 7;
const MIST_BOLT_MOTES = 8;

const MistLightning = memo(function MistLightning({ bolt, reduceMotion, onDone }: { bolt: MistBolt; reduceMotion: boolean; onDone: (id: number) => void }) {
  ...
  return <View pointerEvents="none" style={StyleSheet.absoluteFill}>
    {reduceMotion ? null : Array.from({ length: MIST_BOLT_SEGMENTS }, (_, segment) => <MistBoltSegment key={segment} t={t} a={a} b={b} segment={segment} thickness={thickness} />)}
    <MistImpact t={t} at={b} size={bolt.to.width} />
  </View>;
});

/** One stretch of the zigzag; the ends stay pinned to the two cells, the kinks jitter as it flickers. */
function MistBoltSegment({ t, a, b, segment, thickness }: { t: SharedValue<number>; a: { x: number; y: number }; b: { x: number; y: number }; segment: number; thickness: number }) {
  ...
  return <Animated.View style={[styles.mistBolt, style]}><View style={styles.mistBoltCore} /></Animated.View>;
}

/** The impact: a bright ring pulsing out of the struck cell, and motes thrown from it. */
function MistImpact({ t, at, size }: { t: SharedValue<number>; at: { x: number; y: number }; size: number }) {
  ...
  return <>
    <Animated.View style={[styles.mistImpact, { left: at.x - size / 2, top: at.y - size / 2, width: size, height: size, borderRadius: size / 2 }, ring]} />
    {Array.from({ length: MIST_BOLT_MOTES }, (_, index) => <MistImpactMote key={index} t={t} at={at} size={size} index={index} />)}
  </>;
}

function MistImpactMote({ t, at, size, index }: { t: SharedValue<number>; at: { x: number; y: number }; size: number; index: number }) {
  const style = useAnimatedStyle(() => {
    const q = t.value < MIST_BOLT_REACH ? -1 : (t.value - MIST_BOLT_REACH) / (1 - MIST_BOLT_REACH);
  ...
  return <Animated.View style={[styles.mistMote, { left: at.x - 3, top: at.y - 3 }, style]} />;
}
...
const styles = StyleSheet.create({
  ...
  mistBolt: { position: 'absolute', transformOrigin: 'left center', borderRadius: 5, backgroundColor: 'rgba(255,204,70,0.55)', justifyContent: 'center' },
  mistBoltCore: { height: '34%', width: '100%', borderRadius: 2, backgroundColor: '#FFF7D1' },
  mistImpact: { position: 'absolute', borderWidth: 3, borderColor: '#FFE7A0', backgroundColor: 'rgba(255,238,190,0.22)' },
  mistMote: { position: 'absolute', width: 6, height: 6, borderRadius: 3, backgroundColor: '#FFE9A1' },
