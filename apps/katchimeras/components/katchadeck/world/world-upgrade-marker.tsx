...
/**
 * A hatchable companion's tile: a silhouette of the Egg the Mist is keeping, rays behind it once the
 * tile has woken, warmer once the light is enough. Asleep, it is dim and still. Everything here is a
 * plain view or image: no Reanimated value, no rotation, no repeating animation. A rotating rays sprite
 * in this face hard-crashed the first merge on Steppling's mist board (a Fabric mount conflict inside a
 * Reanimated commit), so the rays are a static graphic and the marker's own pulse is the only motion.
 * The Glow ring below is the same bar every marker draws.
 */
function HatchableEggFace({ state, glowProgress, glowTotal, cost, missingGlow }: {
  ...
  return <>
    <View style={[styles.portraitFrame, styles.eggFrame, asleep ? styles.eggFrameAsleep : ready ? styles.eggFrameReady : null]}>
      {asleep ? null : <Image accessibilityIgnoresInvertColors allowDownscaling={false} cachePolicy="memory" contentFit="contain" pointerEvents="none"
        source={EGG_RAYS_ART} style={[styles.eggRays, ready ? styles.eggRaysReady : null]} transition={0} accessible={false} />}
      <Image accessibilityIgnoresInvertColors allowDownscaling={false} cachePolicy="memory-disk" contentFit="contain"
...
const styles = StyleSheet.create({
  ...
  // Static rays, centred behind the silhouette and larger than the well so they read as light from inside it.
  eggRays: { position: 'absolute', width: 92, height: 92, left: -19, top: -19, opacity: 0.55 },
  eggRaysReady: { opacity: 0.9 },
