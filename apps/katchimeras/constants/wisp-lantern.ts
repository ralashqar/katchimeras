...
import { WISP_RARITY } from './wisp-rarity';
...
export const RARE_PACK = 'lantern-radiant';
export const ORDINARY_PROTECTION = 'lantern-visitors:ordinary';
export const LANTERN_PACKS: readonly WispPackDefinition[] = [
  ...
  { id: ORDINARY_PACK, version: 1, collectionId: LANTERN_COLLECTION.id, scope: 'local-lantern-v1', slots: 3,
    name: 'Lantern Visitors', protectionGroup: ORDINARY_PROTECTION,
    pool: LANTERN_VISITORS.map(id => ({ id, weight: id === 'crystal' ? 10 : 18 })), distinct: false, guaranteeAfterDryPacks: 2 },
  { id: RARE_PACK, version: 1, collectionId: LANTERN_COLLECTION.id, scope: 'local-lantern-v1', slots: 3,
    name: 'Radiant Lantern Pack', protectionGroup: 'lantern-visitors:rare', guaranteedRarity: { slot: 2, minimum: 'rare' },
    pool: LANTERN_VISITORS.map(id => ({ id, weight: id === 'crystal' ? 10 : 18 })), distinct: false, guaranteeAfterDryPacks: 2 },
];
export function lanternPackDefinition(id: string, version?: number) {
  const resolvedVersion = version ?? Math.max(...LANTERN_PACKS.filter(p => p.id === id).map(p => p.version));
  const definition = LANTERN_PACKS.find(p => p.id === id && p.version === resolvedVersion);
...
export function validateLanternPacks(catalog: readonly WispCatalogItem[]) {
  for (const pack of LANTERN_PACKS) {
    ...
    for (const entry of pack.pool) {
      ...
      if (!wisp || wisp.availability !== 'ready' || !wisp.packEligible || !['cosmetic', 'seasonal'].includes(wisp.semanticClass) || entry.weight <= 0) throw new Error(`Invalid visitor pack entry: ${entry.id}`);
    }
    if (pack.guaranteedRarity && (!Number.isInteger(pack.guaranteedRarity.slot) || pack.guaranteedRarity.slot < 0 || pack.guaranteedRarity.slot >= pack.slots || !pack.pool.some(entry => WISP_RARITY[catalog.find(w => w.id === entry.id)!.rarity].rank >= WISP_RARITY[pack.guaranteedRarity!.minimum].rank))) throw new Error('Impossible rarity guarantee');
  }
}
