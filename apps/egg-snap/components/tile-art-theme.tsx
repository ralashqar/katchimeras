...
export function TileArtTheme({clear = false, children}: {clear?: boolean; children: ReactNode}) {
  ...
  const bombOverlay = useImage(require('../assets/shell-v1/bomb-overlay.png'));
  const appearance = useMemo<TileAppearance | undefined>(() => normal && readable && bombOverlay ? {
    id: clear ? 'egg-shell-clear-v1' : 'egg-shell-v1', radius: .2, spriteSize: 128, displayFontFamily: 'EggDisplay',
    atlas: clear ? readable : normal, highReadability: clear,
    cells: clear ? CLEAR_BLOCK_IMAGES : BLOCK_IMAGES, symbols: BLOCK_SYMBOLS,
    shieldCell: require('../assets/shell-v1/armour-silver.png'),
    shieldNumberColor: '#343940',
    bombOverlay, bombProjectileColumn: 7,
  } : undefined, [normal, readable, clear, bombOverlay]);
