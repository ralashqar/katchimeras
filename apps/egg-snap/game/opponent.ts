...
export function choosePlacement(run: SlotRunState, accurate: boolean, elapsedMs: number, roll = 0): SlotAction | null {
  ...
  // A non-overlapping origin cannot fill a cell or chip armour. Keep group-origin
  // duplicates and row-major order: both affect the seeded partial-choice weights.
  const targets = new Set(run.beat.groups.flatMap(g => g.cells));
  for (const piece of pieces) {
    ...
    for (const origin of origins) {
      if (!piece.cells.some(c => {
        const row = origin.row + c.row, column = origin.column + c.column;
        return row >= 0 && row < run.grid.rows && column >= 0 && column < run.grid.cols && targets.has(row * run.grid.cols + column);
      })) continue;
