...
} from './cinematic-gesture';
import type { ImageSourcePropType } from 'react-native';
import { useExplorationEnvironmentProgressionStage } from './progression-context';
export function createCinematicEnvironment<BackgroundKey extends string>({todayScene, backgrounds: TODAY_EXPLORATION_BACKGROUND_SOURCES, progressions: EXPLORATION_ENVIRONMENT_PROGRESSION_SOURCES}: {todayScene:{homeExplorationBackground:{_comment:string;eggContactYRatio:number;eggWidthViewportHeightRatio:number;eggWidthViewportWidthRatio:number;creatureContactYRatio:number;creatureWidthViewportHeightRatio:number;creatureWidthViewportWidthRatio:number;overscrollResistance:number;pageTransitionDurationMs:number;quickSwipe:{minDistance:number;minVelocity:number};resetSpring:{damping:number;mass:number;stiffness:number}}}; backgrounds:Record<BackgroundKey,{source:ImageSourcePropType;recyclingKey:string}>;progressions:Partial<Record<BackgroundKey,readonly {full:ImageSourcePropType;medium:ImageSourcePropType}[]>>}) {
...
function useTodayExplorationBackgroundMotion({
  activeKey,
  canSwipeNext = true,
...
function TodayExplorationBackground({
  backgroundKey,
  contentFit = 'fill',
  environmentStage,
  imageSize,
...
function TodayExplorationPageLayer({
  baseOffsetX = 0,
  children,
  hidden = false,
  holdCentered = false,
...
function TodayExplorationSceneLayer({
  baseOffsetX = 0,
  children,
  hidden = false,
  holdCentered = false,
...
return { useTodayExplorationBackgroundMotion, TodayExplorationBackground, TodayExplorationPageLayer, TodayExplorationSceneLayer };
}
