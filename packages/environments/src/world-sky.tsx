...
export type WorldSkyStyle = { key: string; gradient: readonly [string, string, string]; cloudOpacity: { far: number; middle: number; near: number }; veil: string; horizonGlow: string };
const DEFAULT_STYLE: WorldSkyStyle = { key: 'sunny', gradient: ['#2379C6', '#55A9E2', '#BFEAF6'], cloudOpacity: { far: .72, middle: .78, near: .68 }, veil: 'transparent', horizonGlow: 'transparent' };

const CLOUDS = {
  farBank: require('@incubator/art-world/sky/kingdom-cloud-far-bank-512.webp'),
  midWide: require('@incubator/art-world/sky/kingdom-cloud-mid-wide-512.webp'),
  midTall: require('@incubator/art-world/sky/kingdom-cloud-mid-tall-512.webp'),
  nearBank: require('@incubator/art-world/sky/kingdom-cloud-near-bank-1024.webp'),
} as const;
...
export const KingdomSkyBackground = memo(function KingdomSkyBackground({
  ...
  const motionEnabled = active && kingdomSkyMotionEnabled(active, appIsActive, reduceMotion);
...
export function StaticKingdomSkyBackground({ sky, skyStyle, active = true }: { sky?: { seed: number }; skyStyle?: WorldSkyStyle; active?: boolean } = {}) {
  ...
  return <KingdomSkyBackground camera={camera} sky={sky} skyStyle={skyStyle} active={active} viewport={viewport} />;
}
