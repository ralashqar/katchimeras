...
export function createContentFlowRepository(DATABASE_NAME: string, openDatabaseAsync: typeof SQLite.openDatabaseAsync) {
...
async function loadContentFlowRun(runId: string): Promise<ContentFlowRun | null> {
...
async function listContentFlowRuns(options: { activeOnly?: boolean } = {}): Promise<ContentFlowRun[]> {
  const db = await database();
...
async function saveContentFlowTransition(run: ContentFlowRun, event?: ContentFlowEvent): Promise<void> {
 ...
 */
async function reduceContentFlowRunAtomically(input: {
  runId: string;
...
async function contentFlowEventWasRecorded(eventId: string): Promise<boolean> {
  const db = await database();
  return Boolean(await db.getFirstAsync('SELECT event_id FROM content_flow_events WHERE event_id = ?', [eventId]));
}

async function resetContentFlowJournalForDebug(): Promise<void> {
  await serialized(async () => {
    const db = await database();
    await db.withTransactionAsync(async () => {
...
async function deleteContentFlowRunsForDebug(runIds: readonly string[]): Promise<void> {
...
async function deleteContentFlowRunsForDayForDebug(dayId: string): Promise<void> {
...
async function captureContentFlowJournal(): Promise<{ schemaVersion: 1; runs: ContentFlowRun[] }> {
  return { schemaVersion: 1, runs: await listContentFlowRuns() };
}

async function installContentFlowJournalForDebug(input: { schemaVersion: 1; runs: readonly ContentFlowRun[] }): Promise<void> {
  await resetContentFlowJournalForDebug();
  for (const run of input.runs) await saveContentFlowTransition(run);
}

function subscribeContentFlowJournal(listener: () => void) {
  listeners.add(listener);
  return () => { listeners.delete(listener); };
}

async function flushContentFlowJournal() {
  await writeQueue;
}

return { loadContentFlowRun, listContentFlowRuns, saveContentFlowTransition, reduceContentFlowRunAtomically, contentFlowEventWasRecorded, resetContentFlowJournalForDebug, deleteContentFlowRunsForDebug, deleteContentFlowRunsForDayForDebug, captureContentFlowJournal, installContentFlowJournalForDebug, subscribeContentFlowJournal, flushContentFlowJournal };
}
