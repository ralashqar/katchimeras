...
export function createGameTheme(AppFontFamilies: {fredokaBold:string;manrope:string}) {
return {
  color: {
    canvas: '#2B1B13',
    wood: '#3B2F25',
...
type Widen<T> = T extends string ? string : T extends number ? number : T extends readonly unknown[] ? T : T extends object ? { [K in keyof T]: Widen<T[K]> } : T;
type Theme = ReturnType<typeof createGameTheme>;
export type GameTheme = Omit<Widen<Theme>, 'type'> & {type: {[K in keyof Theme['type']]: TextStyle}};
export type GameSurfaceTone = keyof GameTheme['surface'];
