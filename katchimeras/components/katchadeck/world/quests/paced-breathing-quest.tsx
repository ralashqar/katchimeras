...
type BreathingPatternId = 'bedrotte-calm-v1' | 'mendle-soften-v1';
export function PacedBreathingQuest({ config, patternId, onAttemptStart, onAttemptCancel, onComplete, onRunningChange }: QuestProps<Config> & { patternId: BreathingPatternId }) {
  ...
  const mendle = patternId === 'mendle-soften-v1';
  if (!started) return <QuestExperiencePreview eyebrow={mendle ? 'Mendle' : 'Bedrotte'} title={mendle ? 'Soften with Mendle' : 'Breathe with Bedrotte'} body={mendle ? 'Follow a few gentle breaths without asking the feeling to disappear.' : 'Hold while the glow grows. Release and breathe out as it settles.'} icon={mendle ? 'heart.fill' : 'moon.stars.fill'} mediaLabel="A breathing glow" actionLabel="Begin breathing" onAction={start} />;
  if (state.completed && attempt.current) return <ExperienceResult success title={mendle ? 'A gentler moment' : 'A quieter moment'} body={mendle ? 'You made room for every slow breath.' : 'You stayed for every slow breath.'} metric={`${config.cycles} cycles`} onComplete={() => onComplete(attempt.current!, { kind: 'paced_breathing', success: true, completedCycles: config.cycles, durationMs: Date.now() - startedAt.current })} />;
