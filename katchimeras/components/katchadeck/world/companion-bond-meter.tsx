...
export function CompanionBondMeter({ name, progress }: { name: string; progress: CompanionBondProgress }) {
  ...
  useEffect(() => {
    const percent = Math.max(progress.totalPoints > 0 ? 4 : 0, progress.ratio * 100);
    animatedProgress.value = reduceMotion
      ? percent
      : withTiming(percent, { duration: KatchaUI.motion.progress });
  }, [animatedProgress, progress.ratio, progress.totalPoints, reduceMotion]);
  ...
  return (
    <View
      ...
      <View style={[styles.level, { backgroundColor: `${tokens.accent}32`, borderColor: tokens.border }]}>
        <ThemedText style={styles.levelText} lightColor={tokens.text} darkColor={tokens.text}>Lv {progress.level}</ThemedText>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  level: { alignItems: 'center', borderRadius: 9, borderWidth: 1, boxShadow: '-1px 2px 4px rgba(58,38,18,0.14), inset 0 1px 0 rgba(255,252,234,0.68)', justifyContent: 'center', minHeight: 28, minWidth: 36, paddingHorizontal: 5 },
