...
def main():
    ...
    spec = json.load(open(os.path.join(ROOT, 'data/katchimeras/creature-variants.json'), encoding='utf-8'))
    creature = spec['creatures'].get(args.creature)
    if not creature:
        sys.exit(f'no variant spec for {args.creature}')

    ref = args.ref_url or (host_ref(os.path.join(ROOT, args.ref_src)) if args.ref_src else None)
    if not ref:
    ...
    out_dir = os.path.join(ROOT, 'assets/images/katchimeras/variant-grids')
