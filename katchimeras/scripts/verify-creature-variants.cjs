...
const data = JSON.parse(
  fs.readFileSync(path.join(projectRoot, 'data/katchimeras/creature-variants.json'), 'utf8')
);
...
for (const [key, creature] of Object.entries(data.creatures)) {
  ...
  const dir = path.join(projectRoot, 'assets/images/katchimeras/cutouts/variants', creature.visualKey);
