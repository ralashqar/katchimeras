...
def render_with_character(
    ...
    cutout_path = ROOT / "assets" / "images" / "katchimeras" / "cutouts" / f"{character}.png"
...
def main() -> None:
    ...
    prepared_dir = (ROOT / args.prepared_dir).resolve() if args.prepared_dir else None
    output = (
        (ROOT / args.output_dir).resolve()
        if args.output_dir
        else ROOT / ".tmp" / "haven-progressions" / character / ("qa-prepared" if prepared_dir else "qa")
    )
    ...
    print(f"DONE {output.relative_to(ROOT) if output.is_relative_to(ROOT) else output}")
