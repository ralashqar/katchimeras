...
def cmd_object_fp(args):
    ...
    for key in keys:
        ...
        raw = os.path.join(ROOT, 'assets', 'images', 'katchimeras', 'world', folder, f'_fp_{key}_tmp.png')
        urllib.request.urlretrieve(matted, raw)
        ...
        im.save(os.path.join(ROOT, 'assets', 'images', 'katchimeras', 'world', folder, f'{key}.png'))
        os.remove(raw)
...
def cmd_tile_object(args):
    ...
    for key in keys:
        ...
        obj.save(os.path.join(ROOT, 'assets', 'images', 'katchimeras', 'world', folder, f'{key}.png'))
        print(f'  saved {folder}/{key}.png')
...
def _run_object_grids(folder, out_dir, model, keys):
    ...
    for bi in range(0, len(keys), batch_n):
        ...
        Image.open(io.BytesIO(urllib.request.urlopen(url).read())).save(
            os.path.join(ROOT, 'assets', 'images', 'katchimeras', 'world', f'_grid_raw_{folder}.png'))
...
def cmd_object_grid(args):
    ...
    out_dir = os.path.join(ROOT, 'assets', 'images', 'katchimeras', 'world', folder)
...
def cmd_fence_strip(args):
    ...
    out = os.path.join(ROOT, 'assets', 'images', 'katchimeras', 'world', 'props')
...
def cmd_fence(args):
    out_dir = os.path.join(ROOT, 'assets', 'images', 'katchimeras', 'world', 'props')
