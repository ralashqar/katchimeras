...
LAYOUT_ROOT = ROOT / "data" / "local-environments"
ASSET_ROOT = ROOT / "assets" / "images" / "katchimeras" / "environments"
...
def base_path_for(env_dir: Path) -> Path:
    for name in ("base.png", "base.jpg", "base.webp"):
        ...
        if path.exists():
            return path
    raise FileNotFoundError(f"No base image found in {env_dir.relative_to(ROOT)}")
