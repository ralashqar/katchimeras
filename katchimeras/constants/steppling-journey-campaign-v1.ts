...
export function legacyStepplingEpisodeFlow(number: number): ContentFlowDefinition {
  ...
  const nodes: ContentFlowNode[] = [
    { id: 'opening', kind: 'scene', capability: 'journey.reflection', surface: 'companion', sceneId: `${stepplingEpisodeId(number)}:opening`, payload: { text: day.prompt, choices: day.choices },
      actions: day.choices.map(([id]) => ({ id, next: 'participation', set: { preference: id } })) },
    { id: 'participation', kind: 'scene', capability: 'journey.reflection', surface: 'companion', sceneId: `${stepplingEpisodeId(number)}:participation`,
      payload: { text: 'What have you had room for? An honest answer is enough.', choices: [['walk', 'I took a walk'], ['adapted', 'I moved in a way that suited me'], ['rest', 'I made room to rest'], ['not_yet', 'It has not happened yet']] },
      actions: ['walk', 'adapted', 'rest', 'not_yet'].map((id) => ({ id, next: 'activity', set: { participation: id } })) },
    ...
    { id: 'resolution', kind: 'scene', capability: 'journey.reflection', surface: 'companion', sceneId: `${stepplingEpisodeId(number)}:resolution`, payload: { text: day.resolution, choices: [['continue', 'Rest, Steppling']] }, actions: [{ id: 'continue', next: 'rest' }] },
  ...
  return { id: stepplingEpisodeId(number), version: 1, entryNodeId: 'opening', metadata: { kind: 'journey_day', familyId: 'steppling', chapterId: STEPPLING_CHAPTER_ID, number, title: day.title }, nodes };
}
