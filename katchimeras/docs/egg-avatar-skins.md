...
Hat calibration is also data-driven. Each ready entry in `data/egg-avatar/hats.json` owns a residual `presentation` transform; `EggAvatarArtwork` composes it with the selected body's transform. When a hat is too large, reduce its residual scale and recompute `offsetY` so the presented lower contact edge remains fixed near normalized `y = 0.34`. Ordinary caps, hoods, bands, and crowns should read close to the egg's roughly 1100–1200 px crown width instead of filling the broader 1392 px safety box. Purposeful silhouettes such as sunhats, antlers, horns, headphones, and earmuffs may retain controlled overhang. Keep the manifest presentation identical to the catalog, increment the item version, regenerate the catalog, and review `npm run art:egg-avatar-hats:preview` across every body before approval.
...
Faces use the Classic face layer as the exact full-canvas edit target, with Baristabbit and the runtime Today environment as locked style references. The edit target and generated face use a perfectly flat `#FF00FF` background before BiRefNet Heavy so dark pupils and eye outlines cannot be mistaken for a black backdrop; magenta is forbidden inside face features. Promotion keeps BiRefNet's exterior edge authoritative, suppresses only edge pixels positively identified as chroma magenta by red/blue channel dominance, then uses border-connected flood-fill analysis to restore only fully enclosed transparent tears from the untouched GPT source RGB (for example a pupil centre). Damage connected to the exterior is never inferred or hand-painted: regenerate that face instead. Run `python scripts/generate-egg-avatar-skins.py face-pipeline <id> render --review-only`, inspect `.tmp/egg-avatar-faces-v4/<id>/face-review.png`, and promote with `python scripts/generate-egg-avatar-skins.py face-pipeline <id> promote`. Held props use the same two-stage contract as hats: generate exact viewer-right geometry against the Classic egg, style-map that unchanged geometry with Baristabbit and Today, then run BiRefNet Heavy. Held source pixels remain on their generated full canvas; a catalogued `presentation` scale and offset fits the untouched layer into the canonical right-side slot.
