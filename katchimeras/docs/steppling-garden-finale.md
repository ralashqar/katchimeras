...
The finale uses a single generated portrait, `assets/images/katchimeras/world/ui/ftue-game-loop-baked-v3.png`. It follows the user's five-step Game Loop illustration: Merge & Earn → Upgrade / Discover → Hatch Companions → Bond → Journey with them, then back to merging. Official Mossprout and Steppling art provides the character identities in both the Bond and Journey scenes. Its title, numbered panels, arrows, descriptions and floating-island scenery are all baked into the image. There is no duplicate native title or separate icon/text layout. The image retains its intrinsic aspect ratio without cropping; the sheet scrolls on small screens and supplies the complete reading order as an accessibility label. Only the existing “Let’s explore” completion button remains interactive in the fixed footer. See `ftue-game-loop-art-v3-prompt.md` for the generation prompt and references.
