...
import { BATCH_ONE_QUEST_VARIANTS } from '@/constants/batch-one-quest-variants';
import { BATCH_TWO_QUEST_VARIANTS } from '@/constants/batch-two-quest-variants';
import { BATCH_THREE_QUEST_VARIANTS } from '@/constants/batch-three-quest-variants';
...
function defaultQuestPresentationVariants(definition: QuestDefinition): QuestDefinition['presentationVariants'] {
  if (questActivityLane(definition) !== 'real_life' || !definition.repeatPolicy) return undefined;
  const authored = BATCH_ONE_QUEST_VARIANTS[definition.id]
    ?? BATCH_TWO_QUEST_VARIANTS[definition.id]
    ?? BATCH_THREE_QUEST_VARIANTS[definition.id]
    ?? BATCH_FOUR_QUEST_VARIANTS[definition.id]
    ?? BATCH_FIVE_QUEST_VARIANTS[definition.id]
    ?? BATCH_SIX_QUEST_VARIANTS[definition.id];
  if (authored) return authored;
  return [
    { id: 'original', title: definition.title, hint: definition.hint },
    {
      id: 'fresh-detail',
      title: `Return to: ${definition.title}`,
      hint: `${definition.hint} A brief, ordinary, or adapted version counts when it still meets the quest.`,
    },
    {
      id: 'return-compare',
      title: `Another route: ${definition.title}`,
      hint: `${definition.hint} Use the safest accessible form that fits your time, energy, and circumstances.`,
    },
  ];
}
...
type ProgressiveQuestSpec = {
  ...
  presentationVariants?: QuestDefinition['presentationVariants'];
};
...
function progressiveQuestPack(
  ...
  return Object.fromEntries(specs.map((spec) => [
    spec.id,
    {
      ...
      presentationVariants: spec.presentationVariants,
      repeatPolicy: {
      ...
      goalContribution: {
        ...(SPECIALIST_JOURNEY_ID_BY_FAMILY_ID.has(familyId)
          ? { goalTypeIds: [`${familyId}-direction`] }
          : {}),
        amount: 1,
      },
      eligibility: {
...
const RAW_QUEST_DEFINITIONS: Record<string, QuestDefinition> = {
  ...
  ...progressiveQuestPack('flexel', 'flexel-stronger-rhythm', 'flexel', [
    {
      id: 'quest-flexel-session-note', minimumBondLevel: 1, title: 'Keep one training moment',
      hint: 'Log a real strength, gym, mobility, seated, or adapted training moment and what you chose to do.',
    ...
    semanticNoteQuest({
      ...
      hint: 'Describe a real strength, gym, mobility, seated, or adapted session and one exercise, support, technique, range, or body-response detail.',
      request: 'Describe a real strength, gym, mobility, seated, or adapted training moment and include one concrete training or body-response detail.',
      criteria: ['A real completed or intentionally adapted training moment is described', 'At least one exercise, support, technique, range, resistance, set, repetition, mobility, or body-response detail is included'],
    ...
    }),
    {
      ...
      hint: 'After a real training moment, note one body response and one recovery, adaptation, or rest choice.',
    ...
    semanticNoteQuest({
      ...
      hint: 'Review one real training example, what it showed about fit or technique, and one realistic continuation, adaptation, or pause.',
      request: 'Review the week of strength, gym, mobility, seated, or adapted practice with a concrete example and one realistic continuation, adaptation, recovery choice, or pause.',
      criteria: ['At least one real training example is named', 'A change, difficulty, adaptation, body response, or useful detail is identified', 'One next continuation, adjustment, recovery choice, or pause is stated'],
  ...
  ...progressiveQuestPack('sprintail', 'sprintail-running-rhythm', 'sprintail', [
    {
      id: 'quest-sprintail-run-day', minimumBondLevel: 1, title: 'A movement day around running',
      hint: 'On a day you already choose to run or run-walk, reach 3,000 total steps from all movement. This threshold does not prove a run; skip it if it does not fit.',
      family: 'movement', criteria: [{ fact: 'steps.count', op: 'gte', value: 3000, label: 'Reach 3,000 total steps on a chosen running day' }],
      cooldownDays: 2, weight: 4,
    },
    semanticNoteQuest({
      id: 'quest-sprintail-run-detail', minimumBondLevel: 1, title: 'Keep one run detail',
      hint: 'Describe a real run or run-walk and one pace, interval, duration, route, condition, or body-response detail.',
    ...
    }),
    {
      ...
      hint: 'After a real run or run-walk, note one body or energy response and what recovery, adaptation, or rest you chose.',
    ...
    semanticNoteQuest({
      ...
      hint: 'Review one real run or run-walk, what it showed, and one realistic next step, adaptation, recovery choice, or pause.',
      request: 'Review the week of running with one concrete run or run-walk example, what it showed, and one next step, adaptation, recovery choice, or pause.',
      criteria: ['A completed or intentionally shortened run or run-walk is described', 'A pace, interval, route, condition, body-response, adaptation, or recovery pattern is noticed', 'A realistic next running step, adaptation, recovery choice, or pause is stated'],
  ...
  ...progressiveQuestPack('hooplet', 'hooplet-court-rhythm', 'hooplet', [
    {
      id: 'quest-hooplet-court-note', minimumBondLevel: 1, title: 'Keep one basketball moment',
      hint: 'Log a real basketball practice, shoot-around, game, wheelchair or adapted session, solo skill moment, or guided learning activity.',
      family: 'note', criteria: [{ fact: 'notes.added', op: 'gte', value: 1, label: 'Log a basketball session' }],
      cooldownDays: 2, weight: 4, suggestedActions: ['add_note'],
    },
    semanticNoteQuest({
      id: 'quest-hooplet-skill-detail', minimumBondLevel: 1, title: 'Work one basketball detail',
      hint: 'Describe a real basketball moment and one shot, handle, pass, defensive read, decision, drill, team, or adaptation detail.',
      request: 'Describe a real basketball practice, play, adapted session, or guided skill moment and one specific basketball detail.',
      criteria: ['A real basketball practice, play, adapted session, or guided skill moment is explicit', 'A shot, pass, handle, defensive read, drill, decision, team, equipment, or adaptation detail is described'],
    ...
    {
      id: 'quest-hooplet-team-moment', minimumBondLevel: 2, title: 'Notice shared basketball',
      hint: 'Keep one real moment of communication, support, decision-making, inclusion, or shared play.',
    ...
    semanticNoteQuest({
      ...
      hint: 'Review one real basketball moment, a skill, access, or teamwork pattern, and one next focus, adaptation, or pause.',
      request: 'Review the week of basketball with one concrete example, one skill, access, adaptation, or teamwork pattern, and a next focus or pause.',
      criteria: ['A real basketball moment from the week is named', 'A skill, access, adaptation, enjoyment, or teamwork pattern is identified', 'A next focus, adaptation, recovery choice, or pause is stated'],
  ...
  ...progressiveQuestPack('serveling', 'serveling-rally-rhythm', 'serveling', [
    {
      id: 'quest-serveling-session-note', minimumBondLevel: 1, title: 'Keep one racket-sport moment',
      hint: 'Log a real tennis or racket-sport practice, match, cooperative hit, seated or adapted session, or solo skill moment.',
      family: 'note', criteria: [{ fact: 'notes.added', op: 'gte', value: 1, label: 'Log a racket-sport session' }],
      cooldownDays: 2, weight: 4, suggestedActions: ['add_note'],
    },
    semanticNoteQuest({
      id: 'quest-serveling-rally-detail', minimumBondLevel: 1, title: 'Keep one racket-sport detail',
      hint: 'Describe a real session and one serve, stroke, return, rally, decision, positioning, or adaptation detail.',
      request: 'Describe a real tennis or racket-sport practice, match, cooperative hit, seated or adapted session, or solo skill moment and one specific play detail.',
      criteria: ['A real tennis or racket-sport practice, match, cooperative, adapted, or solo skill moment is explicit', 'A serve, rally, stroke, return, point, decision, positioning, equipment, or adaptation detail is included'],
    ...
    {
      id: 'quest-serveling-reset-note', minimumBondLevel: 2, title: 'Reset between attempts',
      hint: 'Keep a note about one breath, cue, adjustment, pause, or recovery choice between points or attempts.',
    ...
    semanticNoteQuest({
      ...
      hint: 'Review one real session, a skill, access, partner, or composure pattern, and one next focus, adaptation, or pause.',
      request: 'Review the week of racket sport with one concrete session example, a skill, access, partner, adaptation, or composure pattern, and a next focus or pause.',
      criteria: ['A real session from the week is named', 'A serve, rally, stroke, positioning, access, partner, adaptation, match, or composure pattern is identified', 'A next focus, adaptation, recovery choice, or pause is stated'],
  ...
  ...progressiveQuestPack('snuglet', 'snuglet-everyday-care', 'snuglet', [
    {
      id: 'quest-snuglet-care-photo', minimumBondLevel: 1, title: 'Keep a caring moment privately',
      hint: 'Write or record one real human-care moment without including another person’s private details.',
      family: 'note', criteria: [{ fact: 'notes.added', op: 'gte', value: 1, label: 'Keep a private caregiving note' }],
      cooldownDays: 2, weight: 4, suggestedActions: ['add_note', 'record_voice'],
    },
    semanticNoteQuest({
      id: 'quest-snuglet-care-detail', minimumBondLevel: 1, title: 'Name the care that happened',
      hint: 'Describe one real act of human care, the relationship or role involved without identifying details, and the need it addressed.',
      request: 'Describe one real act of human caregiving, the non-identifying relationship or care role, and the need, preference, or choice it supported.',
      criteria: ['A concrete completed or intentionally adapted act of care is described', 'The relationship or care role is clear without requiring identifying details', 'The need, preference, routine, comfort, safety, autonomy, or practical support is identified'],
    ...
    {
      id: 'quest-snuglet-caregiver-pause', minimumBondLevel: 2, title: 'Include the caregiver',
      hint: 'Keep a note about one pause, boundary, adjustment, or request for support that included your needs in the care picture.',
    ...
    semanticNoteQuest({
      ...
      hint: 'Review one real care moment, a repeated need or pressure, and one adjustment, boundary, support request, or unresolved gap.',
      request: 'Review the week of caregiving with one concrete example, a repeated need, choice, pressure, or pattern, and one next adjustment, boundary, support request, or unresolved gap.',
      criteria: ['A real caregiving example is named', 'A recurring need, preference, pressure, connection, support gap, or routine is noticed', 'One next adjustment, boundary, support choice, or honestly unresolved gap is stated'],
  ...
  ...progressiveQuestPack('waglet', 'waglet-shared-routine', 'waglet', [
    {
      ...
      hint: 'Photograph your dog or a dog you care for during a safe, ordinary moment. Do not interrupt rest or create a pose the dog dislikes.',
    ...
    semanticNoteQuest({
      id: 'quest-waglet-care-detail', minimumBondLevel: 1, title: 'Share one dog routine',
      hint: 'Describe a real walk, play, reward-based training, comfort, or care moment and one concrete detail.',
      request: 'Describe a real dog-companionship moment and one concrete activity, choice, body-language, comfort, or care detail without diagnosing it.',
      criteria: ['The note is about the player’s real dog or a dog they care for', 'A completed or adapted walk, play, reward-based training, feeding, grooming, rest, comfort, choice, or shared-routine detail is included'],
    ...
    {
      id: 'quest-waglet-routine-note', minimumBondLevel: 2, title: 'Notice without assuming',
      hint: 'Keep one dog body-language signal, preference, or routine detail and leave room for uncertainty about its meaning.',
    ...
    semanticNoteQuest({
      ...
      hint: 'Review one real dog moment, a routine or preference pattern, and one next care, connection, adaptation, or qualified follow-up choice.',
      request: 'Review the week with a dog using one concrete example, a directly observed routine, preference, behaviour, or connection pattern, and one next care, adaptation, observation, or qualified follow-up choice.',
      criteria: ['A real dog-companionship example is named', 'A directly observed routine, preference, behaviour, comfort, choice, or connection pattern is noticed without requiring a diagnosis', 'One next care, reward-based training, play, adaptation, observation, or qualified follow-up choice is stated'],
  ...
  ...progressiveQuestPack('whiskit', 'whiskit-gentle-attention', 'whiskit', [
    {
      ...
      hint: 'Photograph your cat or a cat you care for during a safe, ordinary moment. Do not interrupt rest, hiding, or disengagement.',
      family: 'photo', criteria: [photoQualityCriterion('subject.cat', 'Capture a cat-companionship moment')],
      cooldownDays: 2, weight: 4, suggestedActions: ['take_photo'],
    },
    semanticNoteQuest({
      id: 'quest-whiskit-enrichment-detail', minimumBondLevel: 1, title: 'Follow the cat’s curiosity',
      hint: 'Describe a real play, enrichment, comfort, observation, or care moment and one concrete detail.',
      request: 'Describe a real cat-companionship moment and one concrete play, enrichment, choice, behaviour, comfort, or care detail without diagnosing it.',
      criteria: ['The note is about the player’s real cat or a cat they care for', 'A completed or adapted play, enrichment, feeding, grooming, rest, comfort, choice, observation, or care detail is included'],
    ...
    {
      id: 'quest-whiskit-pattern-note', minimumBondLevel: 2, title: 'Notice without assuming',
      hint: 'Keep one cat body-language signal, possible preference, resting place, or routine detail and leave room for uncertainty.',
    ...
    semanticNoteQuest({
      ...
      hint: 'Review one real cat moment, a routine or possible preference pattern, and one next care, connection, adaptation, or qualified follow-up choice.',
      request: 'Review the week with a cat using one concrete example, a directly observed behaviour, preference, comfort, or routine pattern, and one next care, adaptation, observation, or qualified follow-up choice.',
      criteria: ['A real cat-companionship example is named', 'A directly observed behaviour, possible preference, comfort, choice, or routine pattern is noticed without requiring a diagnosis', 'One next care, enrichment, adaptation, observation, comfort, or qualified follow-up choice is stated'],
  ...
  ...progressiveQuestPack('steppling', 'steppling-everyday-momentum', 'steppling', [
    semanticNoteQuest({
      id: 'quest-steppling-walk-detail', minimumBondLevel: 1, title: 'Keep one detail from a walk',
      hint: 'After a walk, note one thing you noticed and how you felt afterwards.',
      request: 'Describe a walk you completed. Include one thing you noticed and how you felt afterwards.',
      ...
      retryPrompt: 'Add one thing you noticed on the walk and how you felt afterwards.',
      cooldownDays: 2, weight: 12,
      presentationVariants: [
        { id: 'original', title: 'Keep one detail from a walk', hint: 'After a walk, note one thing you noticed and how you felt afterwards.' },
        { id: 'fresh-detail', title: 'Notice something as you walk', hint: 'On your next walk, notice one sight, sound, or physical feeling. Afterwards, note it and how the walk left you feeling.' },
        { id: 'return-compare', title: 'Look again on a familiar walk', hint: 'After a familiar walk, note what felt the same or different and how you felt afterwards.' },
      ],
    }),
  ]),
  ...progressiveQuestPack('mossprout', 'mossprout-nearby-nature', 'mossprout', [
    semanticNoteQuest({
      ...
      hint: 'Share something specific from a real nearby-nature moment, including a plant or view from indoors.',
      request: 'Describe a real nearby-nature moment and one specific living, seasonal, sensory, weather, or changing detail you noticed.',
      criteria: ['A real outdoor place, window view, plant, garden, park, weather, or other nearby-nature moment is described', 'A specific observed detail is included'],
      exclusions: ['A generic statement about liking nature', 'A future plan to go outside'],
      retryPrompt: 'Add where or how you noticed nature and one specific thing growing, moving, sounding, feeling, or changing.',
      cooldownDays: 2, weight: 12,
    }),
  ]),
  ...
  ...progressiveQuestPack('cheerlet', 'cheerlet-visible-progress', 'cheerlet', [
    semanticNoteQuest({
      id: 'quest-cheerlet-progress-detail', minimumBondLevel: 1, title: 'Why does this effort matter?',
      hint: 'Describe one real effort, change, boundary, ending, or survival moment and why it deserves fair acknowledgement.',
      request: 'Describe one concrete effort, change, boundary, ending, adaptation, or act of getting through and explain why it matters to you.',
      criteria: ['A concrete effort, change, boundary, ending, adaptation, or survival moment is described', 'Its personal meaning or value is included'],
      exclusions: ['Generic positive affirmations with no real event', 'A future hope with no present effort or change'],
      retryPrompt: 'Add what actually happened or what it took, and why that deserves fair acknowledgement.',
      cooldownDays: 2, weight: 12,
    }),
  ]),
  ...progressiveQuestPack('vesperitt', 'vesperitt-intentional-nights', 'vesperitt', [
    semanticNoteQuest({
      id: 'quest-vesperitt-night-detail', minimumBondLevel: 1, title: 'How chosen was the night?',
      hint: 'Describe one real late-night moment, what filled it, and whether it felt chosen, mixed, unavoidable, or drifted.',
      request: 'Describe a real late-night moment, what filled it, and whether it felt chosen, mixed, unavoidable, or like unplanned drift.',
      criteria: ['A real late-night moment or activity is described', 'The player reflects on choice, constraint, intention, or drift'],
      exclusions: ['A general opinion about sleep with no real night', 'A future bedtime plan only'],
      retryPrompt: 'Add what actually filled the late hours and whether it was chosen, mixed, unavoidable, or drifted.',
      cooldownDays: 2, weight: 12,
    }),
  ]),
  ...progressiveQuestPack('shellio', 'shellio-water-connection', 'shellio', [
    semanticNoteQuest({
      id: 'quest-shellio-water-detail', minimumBondLevel: 1, title: 'Keep one real water moment',
      hint: 'Share a real swim, beach, shore, or non-entry water moment and one detail that mattered.',
      request: 'Describe a real swimming, beach, shore, or water-place moment and one concrete body, confidence, safety, sensory, movement, weather, wildlife, or changing detail.',
      criteria: ['A real swim, beach, shore, or non-entry water moment is described', 'A specific body, confidence, safety, sensory, movement, weather, wildlife, or changing detail is included'],
      exclusions: ['A future swimming or beach plan only', 'Naming a pool or body of water without a real experienced moment', 'Claiming unsafe water entry as an achievement'],
      retryPrompt: 'Add what you actually did in or near the water and one specific body, confidence, safety, sensory, movement, or changing detail.',
      cooldownDays: 2, weight: 12,
    }),
  ]),
  ...progressiveQuestPack('flickerbun', 'flickerbun-intentional-watching', 'flickerbun', [
    {
      ...
      minimumBondLevel: 1,
      title: 'Choose a screen story on purpose',
      hint: 'Watch some of a film, episode, documentary, or short you deliberately chose. Stopping partway is allowed.',
      family: 'studio',
    ...
    },
    {
      ...
      minimumBondLevel: 1,
      title: 'Keep one screen-story moment',
      hint: 'Keep a short note about one scene, image, sound, feeling, or idea that stayed with you.',
      family: 'note',
    ...
    },
    {
      ...
      hint: 'Try some of a screen story outside your usual genre, style, place, or viewpoint, then note what felt different.',
      family: 'note',
    ...
    },
    {
      ...
      minimumBondLevel: 3,
      title: 'Read the week’s watching',
      hint: 'Review what you started, stopped, finished, or returned to and which choice felt worth your time.',
      family: 'note',
  ...
  ...progressiveQuestPack('relicoon', 'relicoon-cultural-trail', 'relicoon', [
    {
      ...
      hint: 'Choose an object, artwork, or building and keep one detail about who made, used, kept, or interpreted it.',
      family: 'note',
    ...
    },
    {
      ...
      minimumBondLevel: 1,
      title: 'Explore a cultural place',
      hint: 'Visit an accessible museum, gallery, heritage place, archive, or online collection and keep one detail.',
      family: 'place',
    ...
    },
    {
      ...
      hint: 'Review the objects, places, and stories you followed, including any gaps or uncertainty, and choose one thread to carry onward.',
      family: 'note',
  ...
  ...progressiveQuestPack('encora', 'encora-active-music', 'encora', [
    {
      ...
      minimumBondLevel: 1,
      title: 'Listen for one detail',
      hint: 'Listen with the attention, volume, format, and breaks that feel comfortable, then keep one detail you noticed.',
      family: 'note',
    ...
    },
    {
      ...
      hint: 'Review what you listened to, made, shared, or chose not to hear and keep one direction worth repeating.',
      family: 'note',
  ...
  ...progressiveQuestPack('gatherglow', 'gatherglow-tended-connection', 'gatherglow', [
    {
      ...
      minimumBondLevel: 1,
      title: 'Choose one small reach-out',
      hint: 'If it feels right, send a genuine message and note what made the action manageable. Do not include private details about the other person.',
      family: 'note',
    ...
    },
    {
      ...
      hint: 'Capture one moment that felt genuinely shared. Avoid including another person or their private information without permission.',
      family: 'moment',
    ...
    },
    {
      ...
      minimumBondLevel: 2,
      title: 'Make room for a genuine check-in',
      hint: 'In a conversation that feels safe, notice what helped it feel genuine. You do not need to disclose more than feels comfortable.',
      family: 'note',
    ...
    },
    {
      ...
      hint: 'Review when connection felt mutual, when you needed space, and one social rhythm or boundary worth tending.',
      family: 'note',
  ...
  ...progressiveQuestPack('cheerlet', 'cheerlet-visible-progress', 'cheerlet', [
    {
      ...
      minimumBondLevel: 1,
      title: 'Give one change fair credit',
      hint: 'Keep a short note about one effort, movement, beginning, ending, or act of getting through that deserves fair acknowledgement.',
      family: 'note',
    ...
    },
    {
      ...
      minimumBondLevel: 1,
      title: 'Say what deserves acknowledgement',
      hint: 'Record a brief voice note about an effort, change, milestone, or survival moment worth recognising. Celebration is optional.',
      family: 'voice',
    ...
    },
    {
      ...
      minimumBondLevel: 2,
      title: 'Mark the chapter honestly',
      hint: 'Keep what changed, what it took, and what or who supported you. Include mixed feelings if they belong.',
      family: 'note',
    ...
    },
    {
      ...
      minimumBondLevel: 3,
      title: 'Gather the week’s real progress',
      hint: 'Review effort, support, survival, beginnings, endings, and unfinished movement that deserve fair credit.',
      family: 'note',
  ...
  ...progressiveQuestPack('skylo', 'skylo-local-discovery', 'skylo', [
    {
      ...
      minimumBondLevel: 1,
      title: 'Catch one local detail',
      hint: 'Photograph an accessible local detail that makes a street, building, route, or view distinct.',
      family: 'photo',
    ...
    },
    {
      ...
      minimumBondLevel: 1,
      title: 'Choose one local stop',
      hint: 'Visit an accessible local place, then keep its name and one detail you noticed.',
      family: 'place',
    ...
    },
    {
      ...
      minimumBondLevel: 2,
      title: 'Know one local area better',
      hint: 'Notice a nearby area in a safe, accessible way and keep one note about what made it distinct.',
      family: 'note',
    ...
    },
    {
      ...
      minimumBondLevel: 3,
      title: 'Map the week’s local moments',
      hint: 'Review the places, routes, views, and details you noticed and choose one accessible curiosity to keep.',
      family: 'note',
  ...
  ...progressiveQuestPack('coffee-ritual', 'coffee-ritual-intentional-pause', 'baristabbit', [
    {
      ...
      title: 'Pause for the first sip', hint: 'Capture a coffee, tea, water, or other drink moment you chose to notice.',
  ...
  ...progressiveQuestPack('errandimp', 'errandimp-lighter-loops', 'errandimp', [
    ...
    },
    {
      ...
      title: 'Handle one useful task', hint: 'Complete one maintenance or admin action and note the concrete result.',
  ...
  ...progressiveQuestPack('dawnle', 'dawnle-kinder-beginnings', 'dawnle', [
    {
      ...
      title: 'Catch an early-morning detail', hint: 'If you are already awake and it suits you, photograph one real morning detail before 8am. Do not wake early for this quest.',
    ...
    },
    {
      ...
      title: 'Name what shaped the start', hint: 'Keep a short note about the first action, need, cue, or interruption that shaped your day’s beginning.',
    ...
    },
    {
      ...
      title: 'Prepare one easier beginning', hint: 'Prepare one small cue for your next start, then note whether it helped, did nothing, or no longer fitted.',
    ...
    },
    {
      ...
      title: 'Read the week’s beginnings', hint: 'Review how your days began, including late or disrupted starts. Keep one cue, condition, or adaptation worth remembering.',
  ...
  ...progressiveQuestPack('mendle', 'mendle-gentle-repair', 'mendle', [
    {
      ...
      title: 'Keep one honest check-in', hint: 'Name what is present and what support the next hour may need. “I’m not sure” and “I need human help” are valid.',
    ...
    },
    {
      ...
      title: 'Choose one supportive action', hint: 'Try one safe, familiar action that may offer comfort, grounding, practical relief, or connection, then note the honest effect.',
    ...
    },
    {
      ...
      title: 'Find a more accurate story', hint: 'Notice one harsh or blaming interpretation and write a more accurate version without forcing positivity.',
    ...
    },
    {
      ...
      title: 'Notice what support fitted', hint: 'Review without grading the week: what offered support, what did nothing, what felt worse, and what needs human or professional help.',
  ...
  ...progressiveQuestPack('quietome', 'quietome-chosen-solitude', 'quietome', [
    ...
    },
    {
      ...
      title: 'Choose a low-input pause', hint: 'Take a brief pause with less input if it feels supportive, then note what became noticeable. Stop or seek company if quiet feels worse.',
    ...
    },
    {
      ...
      title: 'Read the week’s reflection', hint: 'Review what became clearer, what stayed open, and when quiet, input, or support fitted best.',
  ...
  'quest-step-sprint': {
    ...
    familyId: 'steppling',
    family: 'movement',
    title: 'One-minute step challenge',
    hint: 'Move at a pace that feels safe and count your steps for one minute.',
    criteria: [],
  ...
  'quest-step-time-trial': {
    ...
    familyId: 'steppling',
    family: 'movement',
    title: 'Step target challenge',
    hint: 'Reach the step target at a pace that feels safe and see how long it takes.',
    criteria: [],
    requiresCapabilities: ['health.steps'],
  ...
  'quest-steppling-gentle-walk': {
    ...
    minimumBondLevel: 1,
    family: 'movement',
    title: 'An everyday walking day',
    hint: 'If it suits you today, reach 4,000 steps through ordinary movement.',
    criteria: [{ fact: 'steps.count', op: 'gte', value: 4000, label: 'Reach 4,000 steps today' }],
    requiresCapabilities: ['health.steps'],
    ...
    presentationVariants: [
      { id: 'original', title: 'An everyday walking day', hint: 'If it suits you today, reach 4,000 steps through ordinary movement.' },
      { id: 'split-across-day', title: 'Let the steps add up', hint: 'If it suits you today, reach 4,000 steps across a few ordinary journeys or short walks.' },
      { id: 'purposeful', title: 'Walk with a purpose', hint: 'If it suits you today, reach 4,000 steps while getting somewhere or doing something useful.' },
    ],
  },
  'quest-steppling-walk-note': {
    ...
    hint: 'After a walk, note any change in your energy, attention, or mood. No change is also useful to notice.',
    ...
    presentationVariants: [
      { id: 'original', title: 'Notice the after-effect', hint: 'After a walk, note any change in your energy, attention, or mood. No change is also useful to notice.' },
      { id: 'short-walk', title: 'Check in after a short walk', hint: 'After a short or practical walk, note how your energy, attention, or mood feels.' },
      { id: 'before-after', title: 'Compare before and after', hint: 'After a familiar walk, note whether anything changed in how you felt. No change is a valid answer.' },
    ],
  },
  'quest-steppling-weekly-review': {
    ...
    minimumBondLevel: 3,
    family: 'note',
    title: 'Look back on your walking week',
    hint: 'Review when walking fitted best, what made it easier, and what you want to carry forward.',
    ...
    presentationVariants: [
      { id: 'original', title: 'Look back on your walking week', hint: 'Review when walking fitted best, what made it easier, and what you want to carry forward.' },
      { id: 'support-pattern', title: 'What supported your walks?', hint: 'Look across the week. Note the time, reason, route, or company that helped most and what you want to repeat.' },
      { id: 'adjust-pattern', title: 'What would make next week easier?', hint: 'Look across the week. Note what made walking difficult and one kind adjustment for next time.' },
    ],
  },
  ...
  'quest-mossprout-green-photo': {
    ...
    hint: 'Photograph accessible nearby greenery, including a plant or a view from indoors.',
  ...
  'quest-mossprout-nature-note': {
    ...
    minimumBondLevel: 1,
    family: 'note',
    title: 'Keep a nearby-nature note',
    hint: 'Keep a short note about a plant, animal, weather, sound, texture, or seasonal change noticed outside or from indoors.',
  ...
  'quest-mossprout-return': {
    ...
    hint: 'Return to a familiar place, plant, or view and note one thing that changed or that you missed before.',
  ...
  'quest-mossprout-weekly-review': {
    ...
    minimumBondLevel: 3,
    family: 'note',
    title: 'Read the week in nature',
    hint: 'Review the nature moments available to you, what you noticed, and one place, view, or detail worth returning to.',
  ...
  'quest-feastle-new-flavour': {
    ...
    hint: 'Try or prepare one unfamiliar flavour, then tag the cuisine or ingredient you explored.',
  ...
  'quest-tasklet-clear-three': {
    ...
    minimumBondLevel: 1,
    family: 'note',
    title: 'Clear up to three small things',
    hint: 'Finish one to three useful loose ends, then keep a quick note about what you chose.',
    criteria: [{ fact: 'notes.added', op: 'gte', value: 1, label: 'Record the useful loose ends you finished' }],
    suggestedActions: ['add_note'],
    repeatPolicy: { cadence: 'anytime', cooldownDays: 3 },
    progression: { journeyId: 'tasklet-focus-journey', stageId: 'momentum' },
    goalContribution: { amount: 1 },
  ...
  'quest-long-walk': {
    ...
    lane: 'real_life',
    minimumBondLevel: 2,
    title: 'A longer walking day',
    hint: 'If a longer walking day suits you, reach 8,000 steps today.',
    criteria: [{ fact: 'steps.count', op: 'gte', value: 8000, label: 'Reach 8,000 steps today' }],
    repeatPolicy: { cadence: 'anytime', cooldownDays: 3 },
    progression: { journeyId: 'steppling-everyday-momentum', stageId: 'walk' },
    goalContribution: { amount: 1 },
    eligibility: { creatureKeys: ['steppling'], cooldownDays: 3, weight: 4 },
    presentationVariants: [
      { id: 'original', title: 'A longer walking day', hint: 'If a longer walking day suits you, reach 8,000 steps today.' },
      { id: 'several-walks', title: 'Build a longer walking day', hint: 'If it suits you, let several walks or journeys add up to 8,000 steps today.' },
      { id: 'one-wander', title: 'Make room for a longer wander', hint: 'If it suits you, use a longer walk to help reach 8,000 steps today.' },
    ],
  },
  'quest-snap-today': {
    id: 'quest-snap-today',
  ...
  'quest-goal-note': {
    ...
    minimumBondLevel: 1,
    family: 'note',
    title: 'Keep one useful movement',
    hint: 'Capture a note about a real action, decision, or ending that changed a goal today.',
  ...
  'quest-early-night': {
    ...
    minimumBondLevel: 2,
    family: 'note',
    title: 'Prepare for rest when planned',
    hint: 'Begin getting ready for rest at a time that fits tonight, then note what made that possible or difficult.',
    criteria: [{ fact: 'notes.added', op: 'gte', value: 1, label: 'Record how you prepared for rest' }],
    suggestedActions: ['add_note', 'record_voice'],
    repeatPolicy: { cadence: 'anytime', cooldownDays: 3 },
    progression: { journeyId: 'sleep-rest-gentle-recovery', stageId: 'practice' },
  ...
  'quest-read-book': {
    ...
    lane: 'real_life',
    minimumBondLevel: 1,
    title: 'Spend time with a book',
    hint: 'Read, listen to, or read along with part of a book. Keep its title and one idea, detail, or feeling that stayed with you.',
  ...
  'quest-pagelet-learning-note': {
    ...
    hint: 'Write or record one thing you learned and why it feels useful, interesting, or worth remembering.',
  ...
  'quest-pagelet-curiosity-note': {
    ...
    hint: 'Keep a question you genuinely care about, one place you looked, and one thing you found.',
  ...
  'quest-pagelet-weekly-review': {
    ...
    hint: 'Review what you read, heard, or learned. Keep one idea and one question you want to carry forward.',
  ...
  'quest-late-capture': {
    ...
    minimumBondLevel: 1,
    family: 'photo',
    title: 'Keep a naturally late moment',
    hint: 'If you are already awake between 11pm and 5am, photograph a safe detail from the night. Do not stay awake or go out for this quest.',
  ...
  'quest-vesperitt-night-note': {
    ...
    minimumBondLevel: 2,
    family: 'note',
    title: 'Name what filled the late hours',
    hint: 'Keep a short note about a real late night, what filled it, and whether it felt chosen, mixed, unavoidable, or drifted.',
  ...
  'quest-vesperitt-next-day-note': {
    ...
    hint: 'After a recent late night, note one effect on energy, mood, attention, body, or plans without treating it as a verdict.',
  ...
  'quest-vesperitt-weekly-review': {
    ...
    hint: 'Review which late nights felt chosen, drifted, or unavoidable and what you want to protect, support, or change.',
    ...
    Object.entries(definitions).map(([id, definition]) => {
      ...
      return [
        id,
        {
          ...
          suggestedActions,
          evidencePolicy,
          presentationVariants: definition.presentationVariants ?? defaultQuestPresentationVariants(definition),
        },
      ];
    })
  );
