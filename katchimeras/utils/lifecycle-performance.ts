...
export type ForegroundSurface = 'today' | 'companion' | 'merge';
...
export function foregroundLifecycleViolations(
  ...
  const maximums: Partial<Record<LifecycleResourceKind, number>> = {
    companion_scene: surface === 'companion' ? 1 : 0,
    companion_sheet: surface === 'companion' ? 1 : 0,
  ...
  if (surface !== 'merge' && active.store_subscription > 0) {
    violations.push(`store_subscription:${active.store_subscription}>0`);
  }
  if (surface !== 'merge' && active.app_state_listener > 0) {
