...
ROOT = str(game_root())
OUT_DIR = str(content_path(ROOT, 'assets', 'images', 'katchimeras', 'world', 'design', 'props'))
VISUALS_TS = str(content_path(ROOT, 'utils', 'world-visuals.ts'))
...
def registered_assets():
    ...
    out = {}
    for key, rel in re.findall(r"(\w+):\s*require\('([^']+)'\)", text):
        out.setdefault(key, str(logical_path(ROOT, resolve_asset_specifier(ROOT, rel))))
    return out
...
def main():
    ...
    if args.asset:
        for key in [k.strip() for k in args.asset.split(',') if k.strip()]:
            ...
            jobs.append({'name': key, 'source_abs': str(content_path(ROOT, assets[key])),
                         'subject': args.subject or f'{key.replace("_", " ")} game building/object'})
    if args.source:
        if not args.name:
            sys.exit('--source needs --name')
        jobs.append({'name': args.name, 'source_abs': str(content_path(ROOT, args.source)),
    ...
    env = {}
    for line in open(str(content_path(ROOT, '.env.local')), encoding='utf-8'):
        line = line.strip()
