...
ROOT = game_root()
MANIFEST_PATH = content_path(ROOT, "data") / "game-hub-art.json"
CANDIDATE_DIR = content_path(ROOT, "design") / "game-hub-art" / "candidates"
ASSET_DIR = content_path(ROOT, "assets") / "images" / "katchimeras" / "game-hub"
REGISTRY_PATH = content_path(ROOT, "constants") / "game-hub-art.generated.ts"
...
def load_env():
    values = {}
    for raw in (content_path(ROOT, ".env.local")).read_text(encoding="utf-8").splitlines():
        line = raw.strip()
...
def validate(manifest):
    issues = []
    seen = set()
    style = content_path(ROOT, manifest["styleReference"])
    if not style.exists():
    ...
    for game in manifest["games"]:
        ...
        seen.add(quest_id)
        identity = content_path(ROOT, game.get("identityReference", ""))
        ...
        for mechanic_reference in game.get("mechanicReferences", []):
            reference_path = content_path(ROOT, mechanic_reference)
...
def call_generate(game, manifest):
    url, key = load_env()
    references = [content_path(ROOT, game["identityReference"]), content_path(ROOT, manifest["styleReference"])]
    references.extend(content_path(ROOT, value) for value in game.get("mechanicReferences", []))
    payload = {
        "renderProfile": {
    ...
    print(f"candidate: {logical_path(ROOT, output)}")
...
def write_registry(manifest):
    ...
    for game in promoted:
        rows.append(f"  '{game['questId']}': require('@incubator/art-game-hub/{game['artworkKey']}.webp'),")
    rows.extend([
        "};",
        "",
    ...
    print(f"registry: {logical_path(ROOT, REGISTRY_PATH)} ({len(promoted)} promoted)")
...
def promote(game, candidate_override=None):
    ...
    if not candidate.is_absolute():
        candidate = content_path(ROOT, candidate)
    ...
    print(f"promoted: {logical_path(ROOT, output)}")
