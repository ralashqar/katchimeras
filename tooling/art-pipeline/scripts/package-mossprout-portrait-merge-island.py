...
ROOT = game_root()
DEFAULT_SOURCE = (
    content_path(ROOT, "design")
    / "square-haven-v1"
    / "mossprout-merge-island-portrait-v1-alpha.png"
)
OUTPUT_ROOT = content_path(ROOT, "assets") / "images" / "katchimeras" / "world" / "square"
...
def main() -> None:
    ...
    source_path = args.source if args.source.is_absolute() else content_path(ROOT, args.source)
    ...
    for name, size, quality in OUTPUTS:
        ...
        print(f"Wrote {logical_path(ROOT, output)} ({size[0]}x{size[1]}, {output.stat().st_size} bytes)")
