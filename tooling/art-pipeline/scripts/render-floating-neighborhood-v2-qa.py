...
OUT_DIR = content_path(ROOT, "design/floating-neighborhood-v2")
WORLD_VIEW = json.loads((content_path(ROOT, "constants/kingdom-world-view.json")).read_text(encoding="utf-8"))
...
def paste_creature(canvas: Image.Image, key: str, center: tuple[float, float]) -> None:
    image = Image.open(content_path(ROOT, f"assets/images/katchimeras/cutouts/{key}.png")).convert("RGBA")
...
def paste_zodiac_familiar(canvas: Image.Image, element: str, center: tuple[float, float]) -> None:
    image = Image.open(content_path(ROOT, f"assets/images/katchimeras/zodiac/familiar_{element}.webp")).convert("RGBA")
    base_size = 58.0
