...
def main() -> None:
    ...
    packager = content_path(ROOT, "scripts") / "package-transparent-hex-tile.py"
    ...
    for stem, runtime_key in ASSETS:
        ...
        if not source_path.exists():
            raise SystemExit(f"Missing generated source: {logical_path(ROOT, source_path)}")
        ...
        if args.check:
            if not alpha_path.exists():
                raise SystemExit(f"Missing alpha master: {logical_path(ROOT, alpha_path)}")
            ...
            for suffix in ("", "_512", "_256"):
                ...
                if not runtime_path.exists():
                    raise SystemExit(f"Missing runtime asset: {logical_path(ROOT, runtime_path)}")
