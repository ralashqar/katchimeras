...
const dataRoot = contentPath(root, 'data', 'egg-avatar');
const outputPath = contentPath(root, 'constants', 'egg-avatar-catalog.generated.ts');
const assetOutputPath = contentPath(root, 'constants', 'egg-avatar-assets.generated.ts');
const categories = [
  ['body', 'bodies.json', 50],
  ['face', 'faces.json', 30],
  ['hat', 'hats.json', 40],
...
function validateItem(item, category, index, seenIds, bodyAccents) {
  ...
  for (const resolution of ['high', 'app', 'thumbnail']) {
    ...
    if (!fs.existsSync(contentPath(root, assetPath))) fail(`${label} is missing ${assetPath}`);
  }
  ...
  if (!fs.existsSync(contentPath(root, highRuntimePath))) {
  ...
  const sections = categories.map(([category]) => {
    const entries = catalogs[category]
      ...
      .map((item) => {
        ...
        return `    ${quote(item.id)}: {\n      fullSource: require(${quote(assetSpecifier(root, refs.app))}),\n      highSource: require(${quote(assetSpecifier(root, highRuntimePath))}),\n      thumbnailSource: require(${quote(assetSpecifier(root, refs.thumbnail))}),\n    },`;
      });
    return `  ${category}: {\n${entries.join('\n')}\n  },`;
  });
