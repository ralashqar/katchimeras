...
MASTER_ROOT = content_path(ROOT, "design") / "mossprout-nature-islands-v1" / "max-level"
...
def package(spec: NatureIslandSpec, *, check: bool) -> tuple[int, int, int]:
    ...
    if not path.exists():
        raise SystemExit(f"Missing Haven nature island: {logical_path(ROOT, path)}")
...
def package_max_level(key: str, *, check: bool) -> int:
    ...
    if not master_path.exists():
        raise SystemExit(f"Missing Level 4 master: {logical_path(ROOT, master_path)}")
    ...
    for size in RUNTIME_SIZES:
        ...
        if check:
            if not path.exists():
                raise SystemExit(f"Missing Level 4 runtime asset: {logical_path(ROOT, path)}")
        ...
        print(f"{logical_path(ROOT, path)}: {size}x{size}, {path.stat().st_size // 1024} KiB")
