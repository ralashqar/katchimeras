...
def main() -> None:
    ...
    candidate = (content_path(ROOT, args.candidate)).resolve()
    ...
    work = content_path(ROOT, ".tmp") / "floating-neighborhood-v2" / stem

    existing = [path for path in (source, alpha) if path.exists() and path.resolve() != candidate]
    if existing and not args.replace:
        joined = ", ".join(str(logical_path(ROOT, path)) for path in existing)
    ...
    pipeline = [
        sys.executable,
        str(content_path(ROOT, "scripts") / "hex-tile-pipeline.py"),
        "--source", str(source),
        "--key", asset_key,
