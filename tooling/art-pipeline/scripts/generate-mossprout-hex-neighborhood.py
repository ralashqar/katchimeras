...
GENERATOR = content_path(ROOT, "scripts") / "generate-katchimera-hex-tile.py"
MATTE_PIPELINE = content_path(ROOT, "scripts") / "hex-tile-pipeline.py"
OUT_ROOT = content_path(ROOT, ".tmp") / "katchimera-hex-tiles"
...
def load_manifest() -> dict:
    ...
    if manifest.get("pipelineId") != LOCK_ID:
        raise SystemExit(f"{logical_path(ROOT, MANIFEST_PATH)} must declare pipelineId={LOCK_ID!r}")
...
def resolve_base(tile: dict, generated: set[str]) -> Path:
    if tile.get("base"):
        return content_path(ROOT, tile["base"])
...
def generate(manifest: dict, keys: list[str], *, dry_run: bool) -> None:
    ...
    for key in keys:
        ...
        guide = content_path(ROOT, tile["guide"])
        for path, label in ((base, "base"), (guide, "guide")):
            if not path.exists():
                raise SystemExit(f"{key}: missing {label} image {logical_path(ROOT, path)}")
        command = [
            sys.executable,
            str(GENERATOR),
            "--visual-key",
        ...
        print(f"{key}: {'dry-run' if dry_run else 'generate'} from {logical_path(ROOT, base)}")
...
def promote(manifest: dict, keys: list[str]) -> None:
    for key in keys:
        ...
        if not candidate.exists():
            raise SystemExit(f"{key}: missing reviewed candidate {logical_path(ROOT, candidate)}")
        records_path = candidate.parent / "candidates.json"
        try:
            records = json.loads(records_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise SystemExit(f"{key}: invalid generation record {logical_path(ROOT, records_path)}: {exc}") from None
        record = records[0] if isinstance(records, list) and len(records) == 1 else None
        ...
        for source_name, destination_name in (
            ...
            if not source.exists():
                raise SystemExit(f"{key}: missing provenance file {logical_path(ROOT, source)}")
            shutil.copy2(source, provenance / destination_name)
        print(f"promoted {key} -> {logical_path(ROOT, destination)}")
...
def rematte(manifest: dict, keys: list[str]) -> None:
    ...
    for key in keys:
        ...
        provenance = {
            ...
            "source": str(logical_path(ROOT, source)).replace("\\", "/"),
        ...
        print(f"rematted {key} -> {logical_path(ROOT, alpha)}")
    subprocess.run([sys.executable, "scripts/package-mossprout-hex-neighborhood.py"], cwd=ROOT, check=True)
    subprocess.run([sys.executable, "scripts/render-mossprout-hex-neighborhood-qa.py"], cwd=ROOT, check=True)
...
def check(manifest: dict) -> None:
    ...
    matte_source = (content_path(ROOT, "scripts") / "package-mossprout-hex-neighborhood.py").read_text(encoding="utf-8")
    ...
    for key in manifest["generationOrder"]:
        if not published_source(key).exists():
            raise SystemExit(f"Missing canonical source: {logical_path(ROOT, published_source(key))}")
    for obsolete in (
        content_path(ROOT, "design") / "mossprout-hex-neighborhood-concepts",
        DESIGN_ROOT / "base-alpha.png",
        DESIGN_ROOT / "qa-base-fill-neighborhood.jpg",
    ):
        if obsolete.exists():
            raise SystemExit(f"Obsolete Mossprout pipeline artifact must stay removed: {logical_path(ROOT, obsolete)}")
    package_source = (content_path(ROOT, "package.json")).read_text(encoding="utf-8")
    for script_name in (
