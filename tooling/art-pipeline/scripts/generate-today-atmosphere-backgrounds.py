...
ROOT = game_root()
SPEC_PATH = content_path(ROOT, "design") / "today-atmosphere-backgrounds" / "manifest.json"
WORK_ROOT = content_path(ROOT, ".tmp") / "today-atmosphere-backgrounds"
ASSET_ROOT = content_path(ROOT, "assets") / "images" / "katchimeras" / "world" / "today-atmosphere"
REGISTRY_PATH = content_path(ROOT, "constants") / "today-atmosphere-background-sources.gen.ts"
...
def sync_registry(spec: dict[str, Any]) -> None:
    ...
    for scene_id, scene in spec["scenes"].items():
        ...
        rows.extend([
            ...
            f"    source: require('{asset_specifier(ROOT, approved['asset'])}'),",
            f"    havenSource: require('{asset_specifier(ROOT, str(logical_path(ROOT, haven_asset_path(approved['asset']))).replace(chr(92), '/'))}'),",
            "  },",
        ])
    body = "\n".join([
...
def cmd_plan(args: argparse.Namespace) -> None:
    ...
    print(json.dumps({
        ...
        "prompt": str(logical_path(ROOT, work / "prompt.txt")).replace("\\", "/"),
        "canvas": spec["canvas"],
    }, indent=2))
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    for index in range(1, args.count + 1):
        ...
        records.append({
            ...
            "previewPath": str(logical_path(ROOT, preview)).replace("\\", "/"),
            "packaged": packaged,
            "createdAt": datetime.now(timezone.utc).isoformat(),
        })
        print(f"  saved {logical_path(ROOT, raw)}", flush=True)
...
def cmd_revise(args: argparse.Namespace) -> None:
    ...
    for index in range(1, args.count + 1):
        ...
        print(f"saved {logical_path(ROOT, raw)}")
...
def cmd_sync_registry(_args: argparse.Namespace) -> None:
    spec = load_spec()
    sync_registry(spec)
    print(logical_path(ROOT, REGISTRY_PATH))
...
def cmd_validate(_args: argparse.Namespace) -> None:
    ...
    for scene_id, scene in spec["scenes"].items():
        ...
        if not isinstance(approved, dict):
            ...
            continue
        path = content_path(ROOT, approved.get("asset", ""))
        if not path.exists():
        ...
        if not haven_path.exists():
            errors.append(f"{scene_id}: missing Haven tier {logical_path(ROOT, haven_path)}")
            continue
