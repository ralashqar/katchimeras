...
ROOT = str(game_root())
CUTOUTS = str(content_path(ROOT, 'assets', 'images', 'katchimeras', 'cutouts'))
BASE_REF = str(content_path(ROOT, 'assets', 'images', 'katchimeras', 'world', 'base', 'base_env2.png'))
...
def matte(image_url, out_name):
    data = call('remove-image-background', {
        ...
        'model': 'BiRefNet_lite', 'operatingResolution': '1024x1024', 'refineForeground': True,
    })
    m = data.get('imageUrl')
    if not m:
