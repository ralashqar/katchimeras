...
def generate(args: argparse.Namespace) -> None:
    ...
    style_board_path = (
        ...
        else content_path(ROOT, "tmp") / "imagegen" / "fal-style-boards" / f"{args.id}.png"
    )
    ...
    metadata = {
        ...
        "requestId": request_id,
        "sourceUrl": image_url,
        "promptFile": str(logical_path(ROOT, prompt_path)),
        "promptSha256": hashlib.sha256(prompt.encode("utf-8")).hexdigest(),
        "references": [str(logical_path(ROOT, path)) for path in references],
        "styleBoard": str(logical_path(ROOT, style_board_path)),
        "output": str(logical_path(ROOT, output_path)),
        "size": args.size,
    ...
    print(f"saved {logical_path(ROOT, output_path)}", flush=True)
    print(f"saved {logical_path(ROOT, metadata_path)}", flush=True)
