...
ROOT = game_root()
SOURCE_DIR = content_path(ROOT, "design") / "mossprout-memory-garden-v1"
GENERATED_GARDEN_DIR = content_path(ROOT, "design") / "mossprout-hex-neighborhood-v1"
HEX_DIR = content_path(ROOT, "assets") / "images" / "katchimeras" / "world" / "hex"
PLANT_DIR = content_path(ROOT, "assets") / "images" / "katchimeras" / "world" / "memory-plants"
...
def package_garden(level: int) -> None:
    ...
    if not generated_alpha.exists():
        raise RuntimeError(
            f"Missing Nano Banana garden master: {logical_path(ROOT, generated_alpha)}. "
