"""Generate the Timber currency icon through the existing FAL Nano Banana pipeline.

Timber is the Sanctuary's building material, earned on Supply Runs. The Glow swirl is the style reference (same chunky
toy family as the currency icons), but Timber is its own shape and colour: two stacked plump honey-wood logs. Sources
and the generation record are kept under design/timber-icon-v1; delete source.png to try again.

    python scripts/generate-timber-icon.py
"""
...
ROOT = game_root()
OUT = content_path(ROOT, 'design/timber-icon-v1')
ART = content_path(ROOT, 'assets/images/katchimeras/merge-world/ui/timber-v1.webp')
...
PROMPT = (
    ...
    'DIFFERENT icon: TWO plump short round wooden logs stacked, one lying on top of the other at a slight angle, '
    'centred and very large. The bark is one solid warm honey-brown with a slightly deeper brown on the shaded side; '
    'each log shows one round cut end facing the viewer in pale warm tan with two simple darker tan rings. At most '
    'four colours: honey-brown, deeper brown, pale tan, darker tan. No wood grain lines, no knots, no leaves, no moss, '
    'no axe, no rope. It must be unmistakable at 20 pixels and clearly different from a golden swirl. Perfectly flat '
    'pure black #000000 background for matting, the whole silhouette inside the canvas with clear padding. No third '
    'object, no ring, no coin, no sparkles, no particles, no aura, no face, no text, no letters, no border, no UI, no '
    'watermark, no floor, no cast shadow.'
)
...
if not source.exists():
    ...
    for attempt in range(1, 4):
        try:
            url = generator.generate_queued_tile(output_name='timber-icon', prompt=PROMPT, base_path=REFERENCE, creature_path=None, quality='high', gpt_size=1024, model='nano')
            break
...
with Image.open(source) as original:
    ...
    # An icon fills its box: crop to the painted logs, then centre it with a small even margin.
