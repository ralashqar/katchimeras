...
ROOT = game_root()
DEFAULT_HATCHLING = content_path(ROOT, "assets/images/katchimeras/hatchlings/base-hatchling-v2.png")
DEFAULT_OUTPUT_ROOT = content_path(ROOT, "assets/images/katchimeras/evolution-grids")
CATALOG_PATH = content_path(ROOT, "data/katchimeras/encounter-katchimeras.json")
GRID_SIDE = 3
...
def load_env() -> tuple[str, str]:
    env: dict[str, str] = {}
    env_path = content_path(ROOT, ".env.local")
...
def infer_final_path(name: str) -> Path | None:
    cutouts = content_path(ROOT, "assets/images/katchimeras/cutouts")
...
def build_three_stage_prompts(display_name: str, description: str) -> tuple[str, str]:
    ...
    child_prompt = (
        ...
        "Do not copy the adult's oversized signature object, carried tool, backpack, sign, marker, counter, uniform, "
        "jacket, vest, boots or shoes. At child age, translate any equipment-based adult motif into only a small "
        "natural body marking, glow shape or undeveloped anatomy cue; reserve the complete object and working outfit "
        "for the unchanged adult destination. "
...
def main() -> None:
    ...
    if not hatchling_path.is_absolute():
        hatchling_path = content_path(ROOT, hatchling_path)
    final_path = (
        None
    ...
    if final_path and not final_path.is_absolute():
        final_path = content_path(ROOT, final_path)
    ...
    if not output_dir.is_absolute():
        output_dir = content_path(ROOT, output_dir)
    ...
    if args.three_stage:
        ...
        if reuse_hatchling_path is not None and not reuse_hatchling_path.is_absolute():
            reuse_hatchling_path = content_path(ROOT, reuse_hatchling_path)
    ...
    if args.refine_grid:
        ...
        if not grid_path.is_absolute():
            grid_path = content_path(ROOT, grid_path)
        if not identity_path.is_absolute():
            identity_path = content_path(ROOT, identity_path)
