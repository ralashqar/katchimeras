...
ROOT = game_root()
REFERENCE = content_path(ROOT, "assets") / "images" / "katchimeras" / "world" / "square" / "mossprout-merge-island.webp"
OUTPUT_DIR = REFERENCE.parent
MASTER_OUTPUT = content_path(ROOT, "design/square-haven-v1/mossprout-merge-island-square-rail-master.png")


def save_webp(image: Image.Image, path: Path, quality: int) -> None:
    image.save(path, "WEBP", quality=quality, method=6, exact=True)
    print(f"Wrote {logical_path(ROOT, path)} ({image.width}x{image.height}, {path.stat().st_size} bytes)")
...
def main() -> None:
    ...
    print(f"Wrote {logical_path(ROOT, MASTER_OUTPUT)} ({candidate.width}x{candidate.height})")
