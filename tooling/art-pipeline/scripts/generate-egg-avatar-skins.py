...
ROOT = game_root()
OUTPUT_DIR = content_path(ROOT, "assets") / "images" / "katchimeras" / "egg-avatars"
...
REVIEW_DIR = content_path(ROOT, ".tmp") / "egg-avatar-skins"
HAT_REVIEW_DIR = content_path(ROOT, ".tmp") / "egg-avatar-hats-v4"
BODY_DRAFT_REVIEW_DIR = content_path(ROOT, ".tmp") / "egg-avatar-body-drafts-v1"
FACE_REVIEW_DIR = content_path(ROOT, ".tmp") / "egg-avatar-faces-v4"
HELD_REVIEW_DIR = content_path(ROOT, ".tmp") / "egg-avatar-held-v2"
MANIFEST_PATH = OUTPUT_DIR / "manifest.json"
BODY_CATALOG_PATH = content_path(ROOT, "data") / "egg-avatar" / "bodies.json"
FACE_CATALOG_PATH = content_path(ROOT, "data") / "egg-avatar" / "faces.json"
HAT_CATALOG_PATH = content_path(ROOT, "data") / "egg-avatar" / "hats.json"
HELD_CATALOG_PATH = content_path(ROOT, "data") / "egg-avatar" / "held.json"
AVATAR_APP_SIZE = 512
AVATAR_HIGH_SIZE = 1536
AVATAR_THUMBNAIL_SIZE = 256
CURRENT_EGG = content_path(ROOT, "assets") / "images" / "katchimeras" / "cutouts" / "egg-base.png"
BARISTABBIT = content_path(ROOT, "assets") / "images" / "katchimeras" / "cutouts" / "baristabbit.png"
CLASSIC_APPROVED = OUTPUT_DIR / "classic.png"
TODAY_RUNTIME_BACKGROUND = content_path(ROOT, "assets") / "images" / "katchimeras" / "world" / "today" / "today_bg.webp"
CUTOUTS_DIR = content_path(ROOT, "assets") / "images" / "katchimeras" / "cutouts"
...
def load_env() -> tuple[str, str]:
    values: dict[str, str] = {}
    path = content_path(ROOT, ".env.local")
    if not path.exists():
...
def generate_planned_body_draft(body_id: str) -> None:
    ...
    (review / "generation.json").write_text(json.dumps({
        ...
        "quality": "low",
        "references": [
            {"path": str(logical_path(ROOT, references[0])).replace("\\", "/"), "role": "exact-faceless-body-edit-target"},
            {"path": str(logical_path(ROOT, references[1])).replace("\\", "/"), "role": "character-art-style"},
            {"path": str(logical_path(ROOT, references[2])).replace("\\", "/"), "role": "runtime-lighting-palette"},
        ],
        "imageUrl": image_url,
...
def refresh_avatar_catalog_registry() -> None:
    subprocess.run(
        ["node", str(content_path(ROOT, "scripts") / "generate-egg-avatar-catalog.cjs")],
        cwd=ROOT,
        check=True,
    )
...
def generate_face_draft(face_id: str) -> None:
    ...
    (review / "generation.json").write_text(json.dumps({
        ...
        "quality": "low",
        "references": [
            {"path": str(logical_path(ROOT, references[0])).replace("\\", "/"), "role": "exact-face-layout-edit-target"},
            {"path": str(logical_path(ROOT, references[1])).replace("\\", "/"), "role": "character-art-style"},
            {"path": str(logical_path(ROOT, references[2])).replace("\\", "/"), "role": "runtime-lighting-palette"},
        ],
        "imageUrl": image_url,
...
def run_face_pipeline(face_ids: tuple[str, ...], *, phase: str, review_only: bool = False) -> None:
    for face_id in face_ids:
        if phase == "render":
            ...
            promote_face_draft(face_id)
    if review_only:
        print(f"Created {len(face_ids)} face review draft(s) under {logical_path(ROOT, FACE_REVIEW_DIR)}.", flush=True)
    else:
...
def run_planned_body_drafts(
    ...
    *,
    ...
    if not review_only:
        ...
        print(f"Created {len(body_ids)} review-only body draft(s) under {logical_path(ROOT, BODY_DRAFT_REVIEW_DIR)}.")
...
def generate(skin_id: str, count: int) -> None:
    ...
    run: dict[str, Any] = {
        ...
        "references": [{"path": str(logical_path(ROOT, path)).replace("\\", "/"), "role": "identity" if index == 0 else "style"} for index, path in enumerate(references)],
        "createdAt": datetime.now(timezone.utc).isoformat(),
        "candidates": [],
    }
    ...
    for index in range(1, count + 1):
        ...
        run["candidates"].append({"index": index, "recordId": record.get("id"), "imageUrl": image_url, "rawPath": str(logical_path(ROOT, path)).replace("\\", "/")})
...
def matte(skin_id: str) -> None:
    ...
    for candidate in run["candidates"]:
        ...
        candidate.update({
            "matteUrl": image_url,
            "mattePath": str(logical_path(ROOT, path)).replace("\\", "/"),
            "mattingModel": MATTING_MODEL,
...
def gameplay_generate(count: int) -> None:
    ...
    for stage in GAMEPLAY_STAGES:
        ...
        run: dict[str, Any] = {
            ...
            "prompt": prompt,
            "model": GENERATION_MODEL,
            "references": [{"path": str(logical_path(ROOT, CLASSIC_APPROVED)).replace("\\", "/"), "role": "identity"}],
            "createdAt": datetime.now(timezone.utc).isoformat(),
            "candidates": [],
        }
        for index in range(1, count + 1):
...
def generate_hat_front(accessory_id: str) -> None:
    ...
    (review / "geometry-generation.json").write_text(json.dumps({
        ...
        "quality": "low",
        "reference": {
            "path": str(logical_path(ROOT, CLASSIC_APPROVED)).replace("\\", "/"),
...
def restyle_hat_front(accessory_id: str, *, use_production_source: bool) -> None:
    ...
    (review / "front-generation.json").write_text(json.dumps({
        ...
        "quality": "low",
        "references": [
            {"path": str(logical_path(ROOT, source)).replace("\\", "/"), "role": "exact-hat-geometry-edit-target"},
            {"path": str(logical_path(ROOT, BARISTABBIT)).replace("\\", "/"), "role": "character-art-style"},
            {"path": str(logical_path(ROOT, TODAY_RUNTIME_BACKGROUND)).replace("\\", "/"), "role": "runtime-lighting-palette"},
        ],
        "imageUrl": image_url,
        "recordId": record.get("id"),
    }, indent=2) + "\n", encoding="utf-8")
...
def approve_hat_front(accessory_id: str) -> None:
    ...
    expected_style_references = {
        ("character-art-style", str(logical_path(ROOT, BARISTABBIT)).replace("\\", "/")),
        ("runtime-lighting-palette", str(logical_path(ROOT, TODAY_RUNTIME_BACKGROUND)).replace("\\", "/")),
    }
...
def run_hat_pipeline(accessory_ids: tuple[str, ...], *, phase: str) -> None:
    if phase == "promote":
        ...
        print(f"Promoted reviewed hat source(s). Verify production compatibility: {logical_path(ROOT, path)}")
        return
    for accessory_id in accessory_ids:
        if phase == "render":
    ...
    print(f"Review the raw source, BiRefNet matte, and composite sheet: {logical_path(ROOT, path)}")
...
def generate_held_front(accessory_id: str) -> None:
    ...
    (review / "geometry-generation.json").write_text(json.dumps({
        ...
        "quality": "low",
        "reference": {"path": str(logical_path(ROOT, CLASSIC_APPROVED)).replace("\\", "/"), "role": "exact-egg-placement-reference"},
        "imageUrl": image_url,
        "recordId": record.get("id"),
    }, indent=2) + "\n", encoding="utf-8")
...
def restyle_held_front(accessory_id: str, *, use_production_source: bool) -> None:
    ...
    (review / "front-generation.json").write_text(json.dumps({
        ...
        "quality": "low",
        "references": [
            {"path": str(logical_path(ROOT, source)).replace("\\", "/"), "role": "exact-held-geometry-edit-target"},
            {"path": str(logical_path(ROOT, BARISTABBIT)).replace("\\", "/"), "role": "character-art-style"},
            {"path": str(logical_path(ROOT, TODAY_RUNTIME_BACKGROUND)).replace("\\", "/"), "role": "runtime-lighting-palette"},
        ],
        "imageUrl": image_url,
...
def save_layer_asset(image: Image.Image, directory: Path, thumbnails: Path, asset_id: str) -> dict[str, Any]:
    ...
    return {
        "png": {"path": str(logical_path(ROOT, png_path)).replace("\\", "/"), "width": 2048, "height": 2048, "sha256": sha256(png_path)},
        "highWebp": {"path": str(logical_path(ROOT, high_path)).replace("\\", "/"), "width": AVATAR_HIGH_SIZE, "height": AVATAR_HIGH_SIZE, "sha256": sha256(high_path)},
        "webp": {"path": str(logical_path(ROOT, webp_path)).replace("\\", "/"), "width": AVATAR_APP_SIZE, "height": AVATAR_APP_SIZE, "sha256": sha256(webp_path)},
        "thumbnail": {"path": str(logical_path(ROOT, thumb_path)).replace("\\", "/"), "width": AVATAR_THUMBNAIL_SIZE, "height": AVATAR_THUMBNAIL_SIZE, "sha256": sha256(thumb_path)},
    }
...
def import_layered_v1(source_dir: Path) -> None:
    ...
    for skin_id in SKINS:
        ...
        entry["baseEditMask"] = {
            ...
            "featherPixels": 12,
            "outsideMaskSource": str(logical_path(ROOT, reference_path)).replace("\\", "/"),
        }
        entry["baseOutputs"] = outputs
...
def approve(skin_id: str, candidate_index: int) -> None:
    ...
    manifest["skins"][skin_id] = {
        ...
        "outputs": {
            "png": {"path": str(logical_path(ROOT, png_path)).replace("\\", "/"), "width": 2048, "height": 2048, "sha256": sha256(png_path)},
            "webp": {"path": str(logical_path(ROOT, webp_path)).replace("\\", "/"), "width": 1024, "height": 1024, "sha256": sha256(webp_path)},
            "thumbnail": {"path": str(logical_path(ROOT, thumb_path)).replace("\\", "/"), "width": 256, "height": 256, "sha256": sha256(thumb_path)},
        },
    }
    MANIFEST_PATH.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
...
def gameplay_approve(crack_one: int, crack_two: int) -> None:
    ...
    for name, image in images.items():
        ...
        outputs[name] = {
            "png": {"path": str(logical_path(ROOT, png_path)).replace("\\", "/"), "size": list(cropped.size), "sha256": sha256(png_path)},
            "webp": {"path": str(logical_path(ROOT, webp_path)).replace("\\", "/"), "size": [webp_width, 1100], "sha256": sha256(webp_path)},
        }
...
def validate(skin_id: str | None = None) -> None:
    ...
    for item in ids:
        for path, expected_size in [
            ...
            if not path.exists():
                errors.append(f"missing {logical_path(ROOT, path)}")
                continue
            metrics = image_metrics(path)
            if tuple(metrics["size"]) != expected_size:
                errors.append(f"wrong size {logical_path(ROOT, path)}: {metrics['size']}")
            if any(metrics["corners"]):
                errors.append(f"non-transparent corner in {logical_path(ROOT, path)}")
            if not 0.2 <= metrics["coverage"] <= 0.7:
                errors.append(f"implausible alpha coverage in {logical_path(ROOT, path)}: {metrics['coverage']}")
        ...
        for path, expected_size in [
            ...
            if not path.exists():
                errors.append(f"missing layered base {logical_path(ROOT, path)}")
                continue
            metrics = image_metrics(path)
            if tuple(metrics["size"]) != expected_size:
                errors.append(f"wrong layered base size {logical_path(ROOT, path)}: {metrics['size']}")
            if any(metrics["corners"]):
                errors.append(f"non-transparent layered base corner in {logical_path(ROOT, path)}")
    ...
    for face_id in ready_face_ids:
        ...
        for path, expected_size in [
            ...
            if not path.exists():
                errors.append(f"missing face layer {logical_path(ROOT, path)}")
                continue
            metrics = image_metrics(path)
            if tuple(metrics["size"]) != expected_size:
                errors.append(f"wrong face layer size {logical_path(ROOT, path)}: {metrics['size']}")
            if any(metrics["corners"]):
                errors.append(f"non-transparent face layer corner in {logical_path(ROOT, path)}")
    ...
    for slot, directory_name, bounds_key in [
        ...
        for accessory_id, accessory_entry in accessory_manifest.get(slot, {}).items():
            for path, expected_size in [
                ...
                if not path.exists():
                    errors.append(f"missing accessory layer {logical_path(ROOT, path)}")
                    continue
                metrics = image_metrics(path)
                if tuple(metrics["size"]) != expected_size:
                    errors.append(f"wrong accessory size {logical_path(ROOT, path)}: {metrics['size']}")
                if any(metrics["corners"]):
                    errors.append(f"non-transparent accessory corner in {logical_path(ROOT, path)}")
