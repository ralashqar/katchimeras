...
ROOT = game_root()
SPEC_PATH = content_path(ROOT, "design") / "exploration-backgrounds" / "manifest.json"
WORK_ROOT = content_path(ROOT, ".tmp") / "exploration-backgrounds"
RUNTIME_KEYS_PATH = content_path(ROOT, "constants") / "today-exploration-background-keys.gen.ts"
RUNTIME_SOURCES_PATH = content_path(ROOT, "constants") / "today-exploration-background-sources.gen.ts"
...
def package_candidate(
    ...
    for label, x in positions.items():
        ...
        qa_paths[label] = str(logical_path(ROOT, path)).replace("\\", "/")
    ...
    return {
        "masterPath": str(logical_path(ROOT, master_path)).replace("\\", "/"),
        "reviewPath": str(logical_path(ROOT, review_path)).replace("\\", "/"),
        "platformGuidePath": str(logical_path(ROOT, guide_path)).replace("\\", "/"),
...
def write_plan(
    ...
    plan = {
        ...
        "model": spec["model"],
        "template": str(logical_path(ROOT, template)).replace("\\", "/"),
        "templateSha256": hashlib.sha256(template.read_bytes()).hexdigest(),
        "styleGuide": (
            str(logical_path(ROOT, style_guide)).replace("\\", "/")
            if style_guide is not None
            else None
        ),
        "guideRole": (
...
def write_runtime_registry() -> None:
    candidates_dir = content_path(ROOT, "design") / "exploration-backgrounds" / "candidates"
    ...
    entries = [
        ...
        *[
            (
                key,
                str(logical_path(ROOT, latest_by_key[key]["runtimePath"])).replace(
                    "\\", "/"
                ),
    ...
    for key, runtime_value, recycling_key in entries:
        entry_lines.extend(
            [
                ...
                f"    source: require('{asset_specifier(ROOT, runtime_value)}'),",
                "  },",
            ]
        )
    ...
    print(
        "refreshed "
        f"{logical_path(ROOT, RUNTIME_KEYS_PATH)} and "
        f"{logical_path(ROOT, RUNTIME_SOURCES_PATH)}"
    )
        ...
        records.append(
            {
                ...
                "url": url,
                "rawPath": str(logical_path(ROOT, raw)).replace("\\", "/"),
                "templateSha256": hashlib.sha256(template.read_bytes()).hexdigest(),
                "styleGuideSha256": (
                    hashlib.sha256(style_guide.read_bytes()).hexdigest()
                    if style_guide is not None
...
def cmd_export(args: argparse.Namespace) -> None:
    ...
    if not records_path.exists():
        raise SystemExit(f"Missing candidate records: {logical_path(ROOT, records_path)}")
    records = json.loads(records_path.read_text(encoding="utf-8"))
    ...
    design_dir = content_path(ROOT, "design") / "exploration-backgrounds" / "candidates"
    runtime_dir = (
    ...
    if existing:
        names = ", ".join(str(logical_path(ROOT, path)) for path in existing)
    ...
    metadata = {
        ...
        "outputs": {
            role: str(logical_path(ROOT, path)).replace("\\", "/")
