...
ROOT = game_root()
MANIFEST = content_path(ROOT, "scripts") / "mossprout-merge-atlas-manifest.json"
SOURCE = content_path(ROOT, "assets") / "images" / "katchimeras" / "merge-world"
...
def main() -> None:
    ...
    for page in manifest["pages"]:
        ...
        print(f"Wrote {logical_path(ROOT, destination)} ({destination.stat().st_size} bytes)")
    RUNTIME_MANIFEST.write_text(json.dumps(runtime, indent=2) + "\n", encoding="utf-8")
    print(f"Wrote {logical_path(ROOT, RUNTIME_MANIFEST)} ({len(seen)} entries)")
