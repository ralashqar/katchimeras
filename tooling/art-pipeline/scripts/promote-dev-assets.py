...
REPO_ROOT = game_root()
OUT_ROOT = content_path(REPO_ROOT, "assets") / "images" / "katchimeras" / "world" / "objects" / "promoted"
...
def load_env() -> tuple[str, str]:
    env_path = content_path(REPO_ROOT, ".env")
    if env_path.exists():
...
def main() -> None:
    ...
    for name in sorted(approved):
        ...
        rel = logical_path(REPO_ROOT, out_path).as_posix()
        print(f"  {remote_path} -> {rel} ({len(webp) / 1024:.0f} KB)")
        # Paths are relative to utils/world-visuals.ts (one level above assets/).
        require_lines.append(f"  {asset_key}_v{index}: require('{asset_specifier(ROOT, rel)}'),")
