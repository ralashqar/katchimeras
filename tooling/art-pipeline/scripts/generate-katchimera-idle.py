...
DEFAULT_MANIFEST = content_path(ROOT, "data/katchimeras/idle-animations.json")
DEFAULT_REGISTRY = content_path(ROOT, "constants/creature-idle-animation-sources.ts")
...
def write_runtime_registry(manifest_path: Path, registry_path: Path) -> None:
    ...
    lines.extend(f"  '{visual_key}': require('{asset_specifier(ROOT, asset)}')," for visual_key, asset, _ in entries)
    lines.extend([
        "};",
        "",
        "const CREATURE_IDLE_FALLBACK_SOURCES: Partial<Record<HomeVisualKey, ImageSourcePropType>> = {",
    ])
    lines.extend(f"  '{visual_key}': require('{asset_specifier(ROOT, fallback)}')," for visual_key, _, fallback in entries)
    lines.extend([
        "};",
        "",
...
def main() -> int:
    ...
    if args.matted_video:
        ...
        env = {**parse_env_file(content_path(ROOT, ".env.local")), **os.environ}
