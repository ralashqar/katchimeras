...
ENV_DIR = content_path(ROOT, "assets") / "images" / "katchimeras" / "environments" / ENV_ID
PROPS_DIR = ENV_DIR / "props"
LAYOUT_PATH = content_path(ROOT, "data") / "local-environments" / f"{ENV_ID}.json"
ART_PATH = content_path(ROOT, "data") / "local-environments" / f"{ENV_ID}.art.json"
STYLE_REF_PATH = content_path(ROOT, "assets") / "images" / "katchimeras" / "environments" / "coffee_cafe" / "base.jpg"
BASE_PATH = ENV_DIR / "base.png"
GUIDE_PATH = ENV_DIR / "guide_slots.png"
TMP_DIR = content_path(ROOT, ".tmp") / "feastle-art"
...
def generate_base(layout: dict[str, Any], art: dict[str, Any], model: str) -> None:
    ...
    print(f"wrote {logical_path(ROOT, BASE_PATH)}")
    subprocess.run([sys.executable, str(content_path(ROOT, "scripts") / "generate-local-environment-guide.py"), ENV_ID], check=True)
