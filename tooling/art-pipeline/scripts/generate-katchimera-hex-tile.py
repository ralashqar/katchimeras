...
def main() -> None:
    ...
    base_path = (content_path(ROOT, args.base or default_base)).resolve()
    ...
    creature_path = (
        (content_path(ROOT, guide_argument)).resolve()
        if guide_argument
        else (content_path(ROOT, f"assets/images/katchimeras/cutouts/{visual_key}.png")).resolve()
    ...
    brief_path = (content_path(ROOT, args.brief)).resolve() if args.brief else None
    brief_data = None
    ...
    for index in range(1, args.count + 1):
        ...
        record = {
            ...
            "theme": theme,
            "prompt": prompt,
            "promptPath": str(logical_path(ROOT, prompt_path)),
            "basePath": str(logical_path(ROOT, base_path)),
        ...
        if brief_path is not None:
            record.update(
                {
                    "briefPath": str(logical_path(ROOT, brief_path)),
        ...
        if args.dry_run:
            ...
            print(f"dry-run {output_name}: {logical_path(ROOT, prompt_path)}")
            continue
        ...
        record.update({"url": image_url, "path": str(logical_path(ROOT, out_path))})
        records.append(record)
