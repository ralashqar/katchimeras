...
export function mergeLessonBoardStep(beat: MergeLessonBeat | undefined, idPrefix = 'merge.lesson', recovery?: {
  ...
  const missingSource = recovery && !recovery.board.some((cell) => !cell.locked && cell.occupant?.kind === 'item' && cell.occupant.definitionId === recovery.requiredDefinitionId);
  ...
  if (beat.kind === 'match') {
    ...
    const to: FtueTarget = { kind: 'board_dream_echo', echoId: beat.echoId };
