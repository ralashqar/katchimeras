...
const SPECIFIC: { re: RegExp; label: string; emoji: string; cuisine?: CuisineFamily }[] = [
  ...
  { re: /\b(sushi|sashimi|maki|nigiri|tempura)\b/, label: 'Sushi', emoji: '🍣', cuisine: 'japanese' },
  { re: /\b(ramen|udon|soba|takoyaki|okonomiyaki|yakitori)\b/, label: 'Ramen', emoji: '🍜', cuisine: 'japanese' },
  { re: /\b(pizza)\b/, label: 'Pizza', emoji: '🍕', cuisine: 'italian' },
  { re: /\b(pasta|noodles?|spaghetti|lasagna|macaroni)\b/, label: 'Pasta', emoji: '🍝', cuisine: 'italian' },
  { re: /\b(burger|hamburger|cheeseburger|fries|french fries|hot ?dog|fast food)\b/, label: 'Burger', emoji: '🍔' },
  { re: /\b(taco|burrito|quesadilla|nachos|enchilada)\b/, label: 'Tacos', emoji: '🌮', cuisine: 'mexican' },
  { re: /\b(curry|tikka|masala|biryani|dal|naan)\b/, label: 'Curry', emoji: '🍛', cuisine: 'indian' },
  { re: /\b(hummus|falafel|shawarma|kebab|pita|tabbouleh)\b/, label: 'Middle Eastern', emoji: '🍽️', cuisine: 'middle_eastern' },
  { re: /\b(dim sum|dumplings?|bao|noodles?|wonton|hot ?pot|peking duck)\b/, label: 'Chinese food', emoji: '🥟', cuisine: 'chinese' },
  { re: /\b(crepe|croissant|baguette|quiche|ratatouille|macaron)\b/, label: 'French food', emoji: '🥐', cuisine: 'french' },
  { re: /\b(gyro|gyros|souvlaki|moussaka|tzatziki|greek salad)\b/, label: 'Greek food', emoji: '🥙', cuisine: 'greek' },
  { re: /\b(salad|salads)\b/, label: 'Salad', emoji: '🥗' },
  { re: /\b(soup|broth|stew|pho)\b/, label: 'Soup', emoji: '🍲' },
  { re: /\b(sandwich|sandwiches|sub|panini|wrap|bagel|baguette)\b/, label: 'Sandwich', emoji: '🥪' },
  { re: /\b(steak|barbecue|bbq|grill|roast|chicken|beef|pork|lamb|skewer)\b/, label: 'Grill', emoji: '🍖' },
...
const LEGACY_FOOD_EMOJI: Record<string, string> = {
  coffee: '☕', tea: '🍵', dessert: '🍰', sushi: '🍣', ramen: '🍜', pizza: '🍕', pasta: '🍝',
  burger: '🍔', taco: '🌮', curry: '🍛', meal: '🍽️', salad: '🥗', soup: '🍲', sandwich: '🥪',
  grill: '🍖', seafood: '🐟', rice: '🍚', drink: '🥤', breakfast: '🍳', fruit: '🍎', bread: '🍞', cheese: '🧀',
};

export function normalizeFoodEmoji(value: string): string {
  return LEGACY_FOOD_EMOJI[value] ?? value;
}
