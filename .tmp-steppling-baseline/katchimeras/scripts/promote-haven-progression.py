...
HEX_ROOT = ROOT / "assets" / "images" / "katchimeras" / "world" / "hex"
...
def run_promotion(stage_key: str, candidate: Path, *, replace: bool, dry_run: bool) -> None:
    command = [
        sys.executable,
        str(ROOT / "scripts" / "promote-floating-neighborhood-v2-tile.py"),
        "--key",
        stage_key,
        "--kind",
        "resident",
...
def rebuild_bounds(*, dry_run: bool) -> None:
    command = [sys.executable, str(ROOT / "scripts" / "generate-hex-tile-bounds.py")]
    print(" ".join(command))
    if not dry_run:
        subprocess.run(command, cwd=ROOT, check=True)
...
def main() -> None:
    ...
    if args.stage is not None:
        ...
        selected = [(stages[args.stage], (ROOT / args.candidate).resolve(), None)]
    else:
    ...
    bounds = ROOT / "constants" / "kingdom-hex-tile-bounds.gen.ts"
    affected.append(bounds)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    backup_root = ROOT / ".tmp" / "haven-progressions" / character / "promotion-backups" / stamp
    ...
    try:
        ...
        restore(snapshot)
        raise
    print(f"DONE. Recovery snapshot: {backup_root.relative_to(ROOT)}")
