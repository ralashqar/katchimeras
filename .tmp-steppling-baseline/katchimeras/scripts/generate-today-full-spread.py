...
SPEC_PATH = ROOT / "design" / "today-full-spread" / "environments.json"
WORK_ROOT = ROOT / ".tmp" / "today-full-spread"
ASSET_ROOT = ROOT / "assets" / "images" / "katchimeras" / "world" / "today"
REGISTRY_PATH = ROOT / "constants" / "today-full-spread-scene-sources.gen.ts"
...
def cmd_plan(args: argparse.Namespace) -> None:
    ...
    plan = {
        ...
        "styleGuide": str(guide.relative_to(ROOT)).replace("\\", "/"),
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    for index in range(1, args.count + 1):
        ...
        print(f"  saved {raw_path.relative_to(ROOT)} and {preview_path.relative_to(ROOT)}")
        ...
        records.append(
            {
                ...
                "source": str(source),
                "identityReference": str(identity_reference.relative_to(ROOT)).replace("\\", "/"),
                "url": image_url,
                "rawPath": str(raw_path.relative_to(ROOT)).replace("\\", "/"),
                "previewPath": str(preview_path.relative_to(ROOT)).replace("\\", "/"),
                "packaged": packaged,
...
def cmd_validate(_args: argparse.Namespace) -> None:
    ...
    for visual_key, environment in sorted(spec["environments"].items()):
        ...
        if not isinstance(reference_value, str) or not (ROOT / reference_value).exists():
