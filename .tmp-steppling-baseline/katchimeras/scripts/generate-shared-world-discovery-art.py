...
def main() -> None:
    ...
    if args.action == "generate":
        ...
        record = {
            ...
            "references": [{"path": p, "sha256": hashlib.sha256((ROOT / p).read_bytes()).hexdigest()} for p in refs],
            "prompt": prompt, "assetKey": tile["assetKey"],
        }
        record_path = folder / "generation.json"
        record_path.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        spec = importlib.util.spec_from_file_location("shared_hex_generator", ROOT / "scripts/generate-katchimera-hex-tile.py")
        generator = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(generator)
        url = generator.generate_queued_tile(
            output_name=tile["assetKey"].replace("_", "-"), prompt=prompt, base_path=ROOT / tile["base"],
            creature_path=ROOT / tile["guide"] if tile.get("guide") else None,
