...
export function GameSurface({ children, contentStyle, density = 'regular', radius, style, tone = 'cream' }: {
  children: ReactNode;
...
export function GameIconWell({ children, size = 40, style, tone = 'cream' }: { children: ReactNode; size?: number; style?: StyleProp<ViewStyle>; tone?: GameSurfaceTone }) {
  const tokens = GameUI.surface[tone];
...
export function GameBadge({ icon, label, style, tone = 'gold' }: { icon?: IconSymbolName; label?: string | number; style?: StyleProp<ViewStyle>; tone?: GameSurfaceTone }) {
  const tokens = GameUI.surface[tone];
...
export function GameRewardChip({ amount, art, icon, tone = 'gold' }: { amount: number; art?: ImageSource | number; icon?: IconSymbolName; tone?: GameSurfaceTone }) {
  const tokens = GameUI.surface[tone];
